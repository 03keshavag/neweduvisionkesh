...
class AutoTeach(Scene):
    def construct(self):
        # ---------- Common Setup ----------
        header = LayoutManager.create_header("Newton’s Third Law – Projectile Motion", "")
        status1 = LayoutManager.create_status_bar("Scene 1")
        self.play(FadeIn(header, shift=UP), FadeIn(status1, shift=DOWN), run_time=1.0)

        # ---------- Scene 1 : Launch Force ----------
        # Canvas
        axes, ground = KinematicsVisualizer.create_canvas(x_max=50, y_max=30)
        self.play(FadeIn(axes, shift=DOWN), FadeIn(ground, shift=UP), run_time=2.0)
        # Particle at origin
        particle = Dot(np.array([0, 0, 0]), radius=0.2, color=YELLOW)
        self.play(FadeIn(particle), run_time=0.5)
        # Initial velocity vector v₀ (20 m/s at 45°)
        theta = 45 * DEGREES
        v0_mag = 20
        v0_vec = v0_mag * np.array([np.cos(theta), np.sin(theta), 0])
        v0_arrow = Arrow(start=particle.get_center(), end=particle.get_center() + v0_vec, buff=0, color=RED)
        v0_label = Text("v₀", font_size=24, color=RED).next_to(v0_arrow, UP)
        self.play(Create(v0_arrow), Write(v0_label), run_time=2.0)
        # Wait to match audio length
        self.wait(9.3)  # 1.0+2.0+0.5+2.0+1.0+9.3 = 16.3 s

        # ---------- Scene 2 : Component Forces ----------
        status2 = LayoutManager.create_status_bar("Scene 2")
        self.play(ReplacementTransform(status1, status2), run_time=0.5)
        ...
        # Component vectors (≈14.14 m/s each)
        comp = v0_mag / np.sqrt(2)
        v0x_vec = np.array([comp, 0, 0])
        v0y_vec = np.array([0, comp, 0])
        v0x_arrow = Arrow(start=particle.get_center(),
                          end=particle.get_center() + v0x_vec, buff=0, color=GREEN)
        v0y_arrow = Arrow(start=particle.get_center(),
                          end=particle.get_center() + v0y_vec, buff=0, color=BLUE)
        v0x_label = Text("vₓ", font_size=22, color=GREEN).next_to(v0x_arrow, RIGHT)
        v0y_label = Text("vᵧ", font_size=22, color=BLUE).next_to(v0y_arrow, UP)
        self.play(Create(v0x_arrow), Write(v0x_label), run_time=1.5)
        self.play(Create(v0y_arrow), Write(v0y_label), run_time=1.5)

        # Reaction force opposite to launch direction
        reaction_vec = -v0_vec / 2  # shorter for visual clarity
        reaction_arrow = Arrow(start=particle.get_center(),
                               end=particle.get_center() + reaction_vec, buff=0, color=ORANGE)
        reaction_label = Text("Fᵣₑₐcₜ", font_size=22, color=ORANGE).next_to(reaction_arrow, LEFT)
        self.play(Create(reaction_arrow), Write(reaction_label), run_time=1.5)

        self.wait(6.7)  # total 15.7 s

        # ---------- Scene 3 : Trajectory & Apex ----------
        status3 = LayoutManager.create_status_bar("Scene 3")
        self.play(ReplacementTransform(status2, status3), run_time=0.5)
        self.clear_stage(preserve=header)
        ...
        g = 9.8
        theta = 45 * DEGREES
        v0x = v0_mag * np.cos(theta)
        v0y = v0_mag * np.sin(theta)
        t_flight = 2 * v0y / g
        path = ParametricFunction(
            lambda t: np.array([v0x * t,
                                v0y * t - 0.5 * g * t ** 2,
                                0]),
            t_range=np.array([0, t_flight]),
            color=WHITE
        )
        ...
        self.play(MoveAlongPath(particle, path), run_time=3.0)

        # Gravity vector (downward)
        gravity_vec = np.array([0, -2, 0])
        gravity_arrow = Arrow(start=particle.get_center(),
                              end=particle.get_center() + gravity_vec, buff=0, color=PURPLE)
        gravity_label = Text("g", font_size=22, color=PURPLE).next_to(gravity_arrow, LEFT)
        self.play(Create(gravity_arrow), Write(gravity_label), run_time=1.0)

        # Apex marker
        t_apex = v0y / g
        apex_point = np.array([v0x * t_apex,
                               v0y * t_apex - 0.5 * g * t_apex ** 2,
                               0])
        apex_marker = Dot(apex_point, radius=0.15, color=RED)
        apex_label = Text("Apex", font_size=22, color=RED).next_to(apex_marker, UP)
        self.play(FadeIn(apex_marker), Write(apex_label), run_time=0.5)

        # Ground reaction force at landing point (origin)
        ground_react_vec = np.array([0, 2, 0])
        ground_react = Arrow(start=np.array([0, 0, 0]),
                             end=np.array([0, 0, 0]) + ground_react_vec,
                             buff=0, color=ORANGE)
        ground_label = Text("N", font_size=22, color=ORANGE).next_to(ground_react, RIGHT)
        self.play(Create(ground_react), Write(ground_label), run_time=0.5)

        self.wait(6.3)  # total 16.3 s

        # ---------- Scene 4 : Range & Summary ----------
        status4 = LayoutManager.create_status_bar("Scene 4")
        self.play(ReplacementTransform(status3, status4), run_time=0.5)
        self.clear_stage(preserve=header)

        axes, ground = KinematicsVisualizer.create_canvas(x_max=50, y_max=10)
        self.play(FadeIn(axes, shift=DOWN), FadeIn(ground, shift=UP), run_time=2.0)

        # Compute range
        R = v0_mag ** 2 * np.sin(2 * theta) / g
        R_approx = round(R, 1)

        # Ground line for range
        range_line = Line(start=np.array([0, 0, 0]),
                          end=np.array([R, 0, 0]), color=YELLOW)
        range_label = Text(f"Range R ≈ {R_approx} m", font_size=24, color=YELLOW).next_to(range_line, UP)
        self.play(Create(range_line), Write(range_label), run_time=1.5)

        # Formula text with numbers
        formula_text = Text(
            f"R = v₀²·sin(2θ)/g = {v0_mag}²·sin(90°)/{g:.1f}",
            font_size=22, color=WHITE
        ).next_to(range_label, DOWN, buff=0.5)
        self.play(Write(formula_text), run_time=1.5)

        # Summary of Newton’s third law
        summary = Text(
            "Every action force has an equal and opposite reaction\n"
            "– launch, gravity, and ground contact all obey this rule.",
            font_size=20, color=LIGHT_GRAY
        ).next_to(formula_text, DOWN, buff=0.7)
        self.play(Write(summary), run_time=2.0)

        self.wait(8.6)  # total 17.1 s

        # End scene
        self.play(FadeOut(VGroup(header, status4, axes, ground, range_line,
                                 range_label, formula_text, summary)), run_time=1.0)