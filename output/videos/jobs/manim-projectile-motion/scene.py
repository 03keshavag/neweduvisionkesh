...
class EduVisionScene(Scene):
    def construct(self):
        ...
        title = Text("Projectile Motion Kinematics", font_size=38, color=BLUE_B).to_edge(UP, buff=0.4)
        subtitle = Text("2D Motion Under Constant Downward Gravity", font_size=20, color=GRAY_B).next_to(title, DOWN, buff=0.15)
        ...
        origin_lbl = Text("(0,0)", font_size=18, color=GRAY).next_to(axes.c2p(0, 0), DL, buff=0.1)
        ...
        v0_lbl = Text("v₀", font_size=22, color=YELLOW).next_to(v0_arrow.get_end(), UR, buff=0.1)
        ...
        vx_lbl = Text("v₀x = v₀ · cos(θ)", font_size=20, color=BLUE_B).next_to(vx_arrow, DOWN, buff=0.15)

        vy_arrow = Arrow(launch_pt, axes.c2p(0, v0y), color=GREEN_B, buff=0, stroke_width=3.5)
        vy_lbl = Text("v₀y = v₀ · sin(θ)", font_size=20, color=GREEN_B).next_to(vy_arrow, LEFT, buff=0.15)
        ...
        apex_lbl = Text("vy = 0 at Apex H", font_size=20, color=GREEN).next_to(apex_pt, UP, buff=0.25)
        ...
        range_lbl = Text("R = v₀² · sin(2θ) / g", font_size=20, color=BLUE_B).next_to(range_brace, DOWN, buff=0.1)

        self.play(Create(range_brace), Write(range_lbl), run_time=1.8)

        # Kinematic Summary Box
        summary_box = VGroup(
            Text("x(t) = v₀x · t", font_size=20, color=BLUE_B),
            Text("y(t) = v₀y · t - ½gt²", font_size=20, color=YELLOW),
        ).arrange(DOWN, buff=0.2).to_edge(RIGHT, buff=0.8).shift(UP * 0.5)
