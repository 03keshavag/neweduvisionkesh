...
class EduVisionScene(Scene):
    def construct(self):
        ...
        title = LayoutManager.create_header("Breadth-First Search (BFS) Traversal", "Graph Traversal")
        self.play(Create(title), run_time=1.2)
        self.wait(0.5)

        nodes = ["A", "B", "C", "D", "E", "F"]
        edges = [["A", "B"], ["A", "C"], ["B", "D"], ["B", "E"], ["C", "F"]]
        positions_dict = {node: np.array([i - len(nodes) // 2, 0, 0]) for i, node in enumerate(nodes)}
        graph = GraphVisualizer(positions_dict, edge_pairs=edges)
        self.play(Create(graph.get_group()), run_time=1.5)
        self.wait(0.5)

        # Scene 2: Enqueue Start Node
        start_node = "A"
        queue = VGroup()
        queue_text = Text("Traversal Queue:").next_to(graph.get_group(), DOWN, buff=1)
        self.play(Write(queue_text), run_time=0.8)
        self.wait(0.2)

        node_a = Text(start_node).next_to(queue_text, RIGHT)
        self.play(FadeIn(node_a), run_time=0.8)
        queue.add(node_a)
        self.wait(0.6)

        # Scene 3: Visit Node and Explore Neighbors
        node_a_green = Text(start_node, color=GREEN).move_to(node_a)
        self.play(ReplacementTransform(node_a, node_a_green), run_time=0.8)
        self.wait(0.2)

        neighbors = [Text("B"), Text("C")]
        for i, neighbor in enumerate(neighbors):
            neighbor.next_to(graph.get_group(), DOWN, buff=1.5).shift((i - len(neighbors) // 2) * 2, 0)
            self.play(FadeIn(neighbor), run_time=0.8)
        self.wait(0.6)

        # Scene 4: Update Queue Display and Continue Traversal
        node_b = Text("B").next_to(queue_text, RIGHT)
        node_c = Text("C").next_to(node_b, RIGHT)
        self.play(FadeIn(node_b), FadeIn(node_c), run_time=0.8)
        queue.add(node_b, node_c)
        self.wait(0.5)

        node_a_green.set_opacity(0.25)
        self.play(node_a_green.animate.set_opacity(0.25), run_time=0.5)
        self.wait(0.5)