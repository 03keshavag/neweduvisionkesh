...
class AutoTeach(Scene):
    def construct(self):
        ...
        status1 = LayoutManager.create_status_bar(
            "Picture five pigeons looking for a place to sit, but there are only four holes available; this sets the stage for the pigeonhole principle."
        )
        self.play(FadeIn(status1), run_time=1.5)

        # Create holes
        hole_positions = [np.array([x, 0.0, 0.0]) for x in [-3, -1, 1, 3]]
        holes = VGroup(*[
            Circle(radius=0.5, color=BLUE_D).move_to(pos)
            for pos in hole_positions
        ])
        self.play(FadeIn(holes), run_time=2.0)

        # Create pigeons
        pigeon_positions = [np.array([x, 2.0, 0.0]) for x in [-4, -2, 0, 2, 4]]
        pigeons = VGroup(*[
            Circle(radius=0.2, color=GRAY).move_to(pos)
            for pos in pigeon_positions
        ])
        self.play(FadeIn(pigeons), run_time=2.0)

        self.wait(4.3)  # total = 9.8 s

        # ==================== SCENE 2 ====================
        # Animate Core Transformation
        self.clear_stage(preserve=header)

        status2 = LayoutManager.create_status_bar(
            "We let the pigeons fly toward the holes; as they settle, one hole inevitably receives two pigeons, illustrating the unavoidable overlap."
        )
        ...
        self.play(FadeIn(holes2), run_time=1.5)

        # Re‑create pigeons (starting above)
        pigeons2 = VGroup(*[
            Circle(radius=0.2, color=GRAY).move_to(pos)
            for pos in pigeon_positions
        ])
        self.play(FadeIn(pigeons2), run_time=1.5)

        # Assign targets (hole0 gets two pigeons)
        targets = [
            holes2[0].get_center(),
            holes2[1].get_center(),
            holes2[2].get_center(),
            holes2[3].get_center(),
            holes2[0].get_center()  # second pigeon to first hole
        ]
        move_animations = [
            pigeons2[i].animate.move_to(targets[i])
            for i in range(5)
        ]
        self.play(*move_animations, run_time=3.0)

        self.wait(3.7)  # total = 10.7 s

        # ==================== SCENE 3 ====================
        # Highlight Key Inflection Point
        self.clear_stage(preserve=header)

        status3 = LayoutManager.create_status_bar(
            "The crowded hole glows, and a vector highlights the mismatch: five items versus four slots, confirming at least one slot holds multiple items."
        )
        self.play(ReplacementTransform(status2, status3), run_time=1.0)

        # Final arrangement (pigeons already in holes)
        holes3 = VGroup(*[
            Circle(radius=0.5, color=BLUE_D).move_to(pos)
            for pos in hole_positions
        ])
        pigeons3 = VGroup(*[
            Circle(radius=0.2, color=GRAY).move_to(targets[i])
            for i in range(5)
        ])
        self.play(FadeIn(holes3), FadeIn(pigeons3), run_time=2.0)

        # Highlight overloaded hole (hole0)
        overloaded_hole = holes3[0]
        self.play(Indicate(overloaded_hole, scale_factor=1.2, color=YELLOW), run_time=2.0)

        # Relationship vector
        vec_start = np.array([-5.5, -1.5, 0.0])
        vec_end   = np.array([5.5, -1.5, 0.0])
        relationship_arrow = Arrow(
            start=vec_start,
            end=vec_end,
            buff=0.0,
            color=GREEN_E
        )
        label_items = Text("5 pigeons", font_size=24, color=WHITE).next_to(relationship_arrow, UP, buff=0.25)
        label_slots = Text("4 holes", font_size=24, color=WHITE).next_to(relationship_arrow, DOWN, buff=0.25)
        vector_group = VGroup(relationship_arrow, label_items, label_slots)
        self.play(FadeIn(vector_group), run_time=1.5)

        self.wait(4.8)  # total = 11.3 s
        ...
        status4 = LayoutManager.create_status_bar(
            "Summarizing, the principle states: if n > m, then some container holds at least ⌈n⁄m⌉ items—here, at least two pigeons share a hole."
        )
        self.play(ReplacementTransform(status3, status4), run_time=1.0)

        # Summary badge (equation card)
        formula_text = "n > m ⇒ ∃ container with ≥⌈n⁄m⌉ items"
        summary_badge = LayoutManager.create_equation_card(
            eq=formula_text,
            label="Pigeonhole Principle"
        )
        self.play(FadeIn(summary_badge), run_time=2.0)

        self.wait(9.1)  # total = 12.1 s