...
class AutoTeach(Scene):
    def construct(self):
        ...
        vel_vec = Arrow(axes.c2p(0, 0), axes.c2p(v0x, v0y), buff=0, color=GREEN)
        self.play(Create(vel_vec), run_time=1.5)
        self.wait(4.9)

        # Scene 2
        self.clear_stage(preserve=header)
        status2 = LayoutManager.create_status_bar("Scene 2")
        self.play(FadeIn(status2), run_time=1.0)
        h_vec = Arrow(axes.c2p(0, 0), axes.c2p(v0x, 0), buff=0, color=BLUE)
        v_vec = Arrow(axes.c2p(0, 0), axes.c2p(0, v0y), buff=0, color=RED)
        g_vec = Arrow(axes.c2p(v0x, v0y), axes.c2p(v0x, v0y - 5), buff=0, color=ORANGE)
        self.play(Create(h_vec), run_time=1.0)
        self.play(Create(v_vec), run_time=1.0)
        self.play(Create(g_vec), run_time=1.0)
        txt_h = Text(f"v₀ₓ = {v0x:.2f} m/s", font_size=24).next_to(h_vec, DOWN)
        txt_v = Text(f"v₀ᵧ = {v0y:.2f} m/s", font_size=24).next_to(v_vec, RIGHT)
        txt_g = Text(f"g = {g:.1f} m/s²", font_size=24).next_to(g_vec, DOWN)
        self.play(Write(txt_h), run_time=0.5)
        self.play(Write(txt_v), run_time=0.5)
        self.play(Write(txt_g), run_time=0.5)
        self.wait(11.2)

        # Scene 3
        self.clear_stage(preserve=header)
        status3 = LayoutManager.create_status_bar("Scene 3")
        self.play(FadeIn(status3), run_time=1.0)
        traj = ParametricFunction(
            lambda t: np.array([v0x * t, v0y * t - 0.5 * g * t**2, 0]),
            t_range=[0, t_flight],
            color=WHITE
        )
        self.play(Create(traj), run_time=1.0)
        particle = Dot(axes.c2p(0, 0), radius=0.15, color=YELLOW)
        self.add(particle)
        self.play(MoveAlongPath(particle, traj), run_time=4.0)
        apex = Dot(axes.c2p(v0x * t_apex, H), radius=0.15, color=PURPLE)
        self.play(Create(apex), run_time=0.5)
        txt_H = Text(f"H = {H:.1f} m", font_size=24).next_to(apex, UP)
        self.play(Write(txt_H), run_time=0.5)
        self.wait(3.3)
        ...
        self.play(FadeIn(status4), run_time=1.0)
        range_line = Line(axes.c2p(0, 0), axes.c2p(R, 0), color=YELLOW)
        self.play(Create(range_line), run_time=1.0)
        txt_R = Text(f"R = {R:.1f} m", font_size=24).next_to(range_line, UP)
        self.play(Write(txt_R), run_time=0.5)
        self.wait(7.6)