...
class AutoTeach(Scene):
    def construct(self):
        # Simple visualizer for an array of numbers
        class SortingVisualizer(VGroup):
            def __init__(self, values, **kwargs):
                super().__init__(**kwargs)
                self.values = values
                # Create a rectangle for each value with its label
                boxes = VGroup()
                for i, val in enumerate(values):
                    rect = Rectangle(width=0.8, height=0.8, color=WHITE, fill_opacity=0.5)
                ...
                self.add(boxes)
                # Center the whole group at origin by default
                self.move_to(np.array([0.0, 0.0, 0.0]))

        self.camera.background_color = "#070b14"

        # ----- Header (persistent) -----
        header = LayoutManager.create_header("Merge Sort", "Recursive sorting algorithm")
        self.add(header)

        # ==================== Scene 1 ====================
        # Clear previous (none) but keep header
        LayoutManager.clear_stage(self, preserve=header)

        # Status bar for scene 1
        status1 = LayoutManager.create_status_bar("Initial array and first split")
        self.add(status1)

        # Visualize the full unsorted array
        full_array = SortingVisualizer([38, 27, 43, 3, 9, 82, 10])
        full_array.move_to(np.array([0.0, 0.0, 0.0]))
        self.play(FadeIn(full_array), run_time=2.0)

        # Visualize left and right halves
        left_half = SortingVisualizer([38, 27, 43, 3])
        right_half = SortingVisualizer([9, 82, 10])

        left_half.next_to(full_array, DOWN, buff=0.5)
        left_half.align_to(full_array, LEFT)

        right_half.next_to(full_array, DOWN, buff=0.5)
        right_half.align_to(full_array, RIGHT)

        self.play(FadeIn(left_half, right_half), run_time=2.0)

        # Arrows indicating the split
        arrow_left = Arrow(
            start=full_array.get_bottom(),
            end=left_half.get_top(),
            buff=0.1,
            color=YELLOW
        )
        arrow_right = Arrow(
            start=full_array.get_bottom(),
            end=right_half.get_top(),
            buff=0.1,
            color=YELLOW
        )
        ...
        # Update status bar
        status1b = LayoutManager.create_status_bar("Array split into left and right halves")
        self.play(ReplacementTransform(status1, status1b), run_time=1.0)

        # Wait to match audio duration
        self.wait(7.7)  # 2+2+1+1+7.7 = 13.7 seconds total

        # ==================== Scene 2 ====================
        LayoutManager.clear_stage(self, preserve=header)

        status2 = LayoutManager.create_status_bar("Recursive splitting")
        self.add(status2)

        # First level halves
        left_pair = SortingVisualizer([38, 27])
        right_pair = SortingVisualizer([43, 3])

        left_pair.move_to(np.array([-2.0, 0.0, 0.0]))
        right_pair.move_to(np.array([2.0, 0.0, 0.0]))

        self.play(FadeIn(left_pair, right_pair), run_time=2.0)

        # Split left pair into singletons
        left_single = SortingVisualizer([38])
        right_single = SortingVisualizer([27])
        left_single.next_to(left_pair, LEFT, buff=0.5)
        right_single.next_to(left_pair, RIGHT, buff=0.5)

        # Split right pair into singletons
        right_left = SortingVisualizer([43])
        right_right = SortingVisualizer([3])
        right_left.next_to(right_pair, LEFT, buff=0.5)
        right_right.next_to(right_pair, RIGHT, buff=0.5)

        # Animate splits
        self.play(
            FadeOut(left_pair),
            FadeIn(left_single, right_single),
            run_time=1.0
        )
        self.play(
            FadeOut(right_pair),
            FadeIn(right_left, right_right),
            run_time=1.0
        )

        # Update status bar
        status2b = LayoutManager.create_status_bar("All subarrays reduced to single elements")
        self.play(ReplacementTransform(status2, status2b), run_time=1.0)

        self.wait(3.0)  # 2+1+1+1+3 = 8.0 seconds total

        # ==================== Scene 3 ====================
        LayoutManager.clear_stage(self, preserve=header)

        status3 = LayoutManager.create_status_bar("Merging subarrays")
        self.add(status3)

        # Unmerged pairs
        left_unsorted = SortingVisualizer([38, 27])
        right_unsorted = SortingVisualizer([43, 3])

        left_unsorted.move_to(np.array([-2.0, 0.0, 0.0]))
        right_unsorted.move_to(np.array([2.0, 0.0, 0.0]))

        self.play(FadeIn(left_unsorted, right_unsorted), run_time=2.0)

        # Merge left pair -> sorted left
        left_sorted = SortingVisualizer([27, 38])
        left_sorted.move_to(left_unsorted.get_center())

        self.play(ReplacementTransform(left_unsorted, left_sorted), run_time=1.5)

        # Merge right pair -> sorted right
        right_sorted = SortingVisualizer([3, 43])
        right_sorted.move_to(right_unsorted.get_center())

        self.play(ReplacementTransform(right_unsorted, right_sorted), run_time=1.5)

        # Final merge of the two sorted halves
        final_merged = SortingVisualizer([3, 27, 38, 43])
        final_merged.move_to(np.array([0.0, 0.0, 0.0]))

        self.play(
            ReplacementTransform(
                VGroup(left_sorted, right_sorted),
                final_merged
            ),
            run_time=2.0
        )
        ...
        # Update status bar
        status3b = LayoutManager.create_status_bar("Subarrays merged into a sorted segment")
        self.play(ReplacementTransform(status3, status3b), run_time=1.0)

        self.wait(3.2)  # 2+1.5+1.5+2+1+3.2 = 11.2 seconds total

        # ==================== Scene 4 ====================
        LayoutManager.clear_stage(self, preserve=header)

        status4 = LayoutManager.create_status_bar("Full merge")
        self.add(status4)

        # Left sorted half (already sorted from previous scene)
        left_full = SortingVisualizer([3, 27, 38, 43])
        left_full.move_to(np.array([-2.5, 0.0, 0.0]))

        # Right sorted half (need to sort [9, 82, 10] first)
        right_unsorted = SortingVisualizer([9, 82, 10])
        right_unsorted.move_to(np.array([2.5, 0.0, 0.0]))

        self.play(FadeIn(left_full, right_unsorted), run_time=2.5)

        # Sort right half internally (merge step)
        right_sorted = SortingVisualizer([9, 10, 82])
        right_sorted.move_to(right_unsorted.get_center())

        self.play(ReplacementTransform(right_unsorted, right_sorted), run_time=1.5)

        # Final full merge
        final_full = SortingVisualizer([3, 9, 10, 27, 38, 43, 82])
        final_full.move_to(np.array([0.0, 0.0, 0.0]))

        self.play(
            ReplacementTransform(
                VGroup(left_full, right_sorted),
                final_full
            ),
            run_time=3.0
        )
        ...
        # Update status bar
        status4b = LayoutManager.create_status_bar("Complete array sorted")
        self.play(ReplacementTransform(status4, status4b), run_time=1.0)

        self.wait(6.8)  # 2.5+1.5+3+1+6.8 = 13.3 seconds total