...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ---------- Header (persistent) ----------
        header = LayoutManager.create_header("Concave Lens", "Concept Overview")
        self.add(header)
        ...
        status1 = LayoutManager.create_status_bar("Setup & Frame of Reference")
        self.play(FadeIn(status1), run_time=1.0)

        # Lens shape (concave lens approximated by thin vertical ellipse)
        lens = Ellipse(width=1.2, height=3.0, color=WHITE).move_to(np.array([0, 0, 0]))
        ...
        ray_y_positions = [0.8, 0.0, -0.8]
        parallel_rays = VGroup()
        for y in ray_y_positions:
            ray = Line(
                start=np.array([-5.5, y, 0]),
                end=np.array([-0.6, y, 0]),
                color=YELLOW,
            )
            parallel_rays.add(ray)
        self.play(
            FadeIn(parallel_rays[0]),
            FadeIn(parallel_rays[1]),
            FadeIn(parallel_rays[2]),
            run_time=1.5,
        )

        # Wait to match audio duration (8.9s total)
        self.wait(3.9)
        ...
        # Update status bar
        status2 = LayoutManager.create_status_bar("Animate core transformation")
        self.play(ReplacementTransform(status1, status2), run_time=1.0)

        # Re‑create lens, axis and incoming rays quickly
        self.play(FadeIn(lens), run_time=0.8)
        self.play(FadeIn(axis), run_time=0.5)
        self.play(FadeIn(parallel_rays), run_time=0.8)

        self.wait(0.5)

        # Refract each ray outward (create new lines and transform)
        refracted_rays = VGroup()
        for i, y in enumerate(ray_y_positions):
            # point where ray exits lens (right side of lens)
            exit_point = np.array([0.6, y, 0])
            # outward direction diverging (simple linear outward)
            end_point = np.array([5.5, y + 1.5 * (i - 1), 0])  # spread them
            new_ray = Line(start=exit_point, end=end_point, color=YELLOW)
            refracted_rays.add(new_ray)

        transforms = []
        for i in range(3):
            transforms.append(Transform(parallel_rays[i].copy(), refracted_rays[i]))
        self.play(*transforms, run_time=1.2)

        # Virtual focus point (left side of lens)
        virtual_focus = Dot(np.array([-2.0, 0, 0]), color=RED)
        self.play(FadeIn(virtual_focus), run_time=0.6)

        # Dotted lines extending backward to virtual focus
        dotted_lines = VGroup()
        for ray in refracted_rays:
            back_line = DashedLine(
                start=virtual_focus.get_center(),
                end=ray.get_start(),
                color=BLUE_D,
            )
            dotted_lines.add(back_line)
        self.play(FadeIn(dotted_lines), run_time=0.8)

        # Wait to match audio duration (8.4s total)
        self.wait(2.2)
        ...
        # Update status bar
        status3 = LayoutManager.create_status_bar("Highlight key inflection point")
        ...
        self.play(FadeIn(parallel_rays), run_time=0.6)
        self.play(FadeIn(virtual_focus), run_time=0.5)

        # Object distance arrow (u)
        obj_start = np.array([-4.5, 0, 0])
        obj_end = np.array([-0.6, 0, 0])
        obj_arrow = Arrow(start=obj_start, end=obj_end, color=GREEN)
        obj_label = Text("u", font_size=24, color=GREEN).next_to(obj_arrow, LEFT, buff=0.25)
        self.play(FadeIn(obj_arrow), FadeIn(obj_label), run_time=0.7)

        # Image distance arrow (v) – virtual image on object's side
        img_center = np.array([-1.0, 0.6, 0])
        img_arrow = Arrow(start=np.array([0.6, 0, 0]), end=img_center, color=ORANGE)
        img_label = Text("v", font_size=24, color=ORANGE).next_to(img_arrow, UP, buff=0.25)
        self.play(FadeIn(img_arrow), FadeIn(img_label), run_time=0.7)

        # Focal length arrow (f) – from lens centre to virtual focus
        focal_arrow = Arrow(start=np.array([0, 0, 0]), end=virtual_focus.get_center(), color=PURPLE)
        focal_label = Text("f", font_size=24, color=PURPLE).next_to(focal_arrow, LEFT, buff=0.25)
        self.play(FadeIn(focal_arrow), FadeIn(focal_label), run_time=0.7)

        # Highlight virtual image point with a glowing circle
        image_glow = Circle(radius=0.2, color=YELLOW).move_to(img_center)
        image_glow.set_fill(YELLOW, opacity=0.5)
        self.play(FadeIn(image_glow), run_time=0.8)
        self.play(image_glow.animate.set_fill(YELLOW, opacity=0.8), run_time=0.5)

        # Wait to match audio duration (10.0s total)
        self.wait(3.0)
        ...
        # Update status bar
        status4 = LayoutManager.create_status_bar("Present summary")
