...
class AutoTeach(Scene):
    def construct(self):
        ...
        header = LayoutManager.create_header("Projectile Motion", "v₀ = 20 m/s, θ = 45°")
        status = LayoutManager.create_status_bar("Scene 1: Setup")
        self.play(FadeIn(header), run_time=1.0)
        self.play(FadeIn(status), run_time=1.0)

        # ---------- Scene 1 (11.1 s) ----------
        axes, ground = KinematicsVisualizer.create_canvas(x_max=50, y_max=20)
        ...
        particle = Dot(axes.c2p(0, 0, 0), color=YELLOW)
        self.play(FadeIn(particle), run_time=0.5)

        vel_vec = Arrow(
            start=axes.c2p(0, 0, 0),
            end=axes.c2p(v0x, v0y, 0),
            buff=0,
            color=RED,
        )
        self.play(Create(vel_vec), run_time=2.0)

        self.wait(4.6)  # total 11.1 s

        # ---------- Scene 2 (16.3 s) ----------
        self.clear_stage(preserve=header)
        status2 = LayoutManager.create_status_bar("Scene 2: Velocity Components")
        self.play(ReplacementTransform(status, status2), run_time=1.0)

        # Re‑create axes and ground for continuity
        self.play(Create(axes), run_time=1.0)
        self.play(Create(ground), run_time=1.0)

        particle2 = Dot(axes.c2p(0, 0, 0), color=YELLOW)
        self.play(FadeIn(particle2), run_time=0.5)

        # Horizontal component
        horiz_vec = Arrow(
            start=axes.c2p(0, 0, 0),
            end=axes.c2p(v0x, 0, 0),
            buff=0,
            color=GREEN,
        )
        self.play(Create(horiz_vec), run_time=2.0)

        # Vertical component
        vert_vec = Arrow(
            start=axes.c2p(v0x, 0, 0),
            end=axes.c2p(v0x, v0y, 0),
            buff=0,
            color=BLUE,
        )
        self.play(Create(vert_vec), run_time=2.0)

        # Labels
        h_label = Text(f"v₀ₓ = {v0x:.2f} m/s", font_size=24).next_to(horiz_vec, DOWN)
        v_label = Text(f"v₀ᵧ = {v0y:.2f} m/s", font_size=24).next_to(vert_vec, LEFT)
        self.play(FadeIn(h_label), FadeIn(v_label), run_time=1.0)

        self.wait(9.3)  # total 16.3 s

        # ---------- Scene 3 (10.2 s) ----------
        self.clear_stage(preserve=header)
        status3 = LayoutManager.create_status_bar("Scene 3: Parabolic Flight")
        self.play(ReplacementTransform(status2, status3), run_time=1.0)

        self.play(Create(axes), run_time=0.5)
        self.play(Create(ground), run_time=0.5)

        particle3 = Dot(axes.c2p(0, 0, 0), color=YELLOW)
        self.play(FadeIn(particle3), run_time=0.5)
        ...
        g_label = Text("g", font_size=24).next_to(g_vec, DOWN)
        self.play(Create(g_vec), FadeIn(g_label), run_time=1.0)
        ...
        moving_particle = Dot(color=YELLOW).move_to(axes.c2p(0, 0, 0))
        trace = TracedPath(moving_particle.get_center, stroke_color=YELLOW, stroke_width=2)

        self.add(moving_particle, trace)
        self.play(
            time_tracker.animate.set_value(t_flight),
            run_time=6.0,
            rate_func=linear,
        )
        self.wait(2.2)  # total 10.2 s

        # ---------- Scene 4 (15.3 s) ----------
        self.clear_stage(preserve=header)
        status4 = LayoutManager.create_status_bar("Scene 4: Apex & Range")
        self.play(ReplacementTransform(status3, status4), run_time=1.0)

        self.play(Create(axes), run_time=0.5)
        self.play(Create(ground), run_time=0.5)

        particle4 = Dot(axes.c2p(0, 0, 0), color=YELLOW)
        self.play(FadeIn(particle4), run_time=0.5)

        # Time tracker for apex and landing
        time_tracker2 = ValueTracker(0.0)

        def pos_func():
            t = time_tracker2.get_value()
            x = v0x * t
            y = v0y * t - 0.5 * g * t ** 2
            return axes.c2p(x, y, 0)

        moving_particle2 = Dot(color=YELLOW).move_to(axes.c2p(0, 0, 0))
        trace2 = TracedPath(moving_particle2.get_center, stroke_color=YELLOW, stroke_width=2)
        self.add(moving_particle2, trace2)

        # Animate to apex (t_apex = v0y / g)
        t_apex = v0y / g
        self.play(
            time_tracker2.animate.set_value(t_apex),
            run_time=3.0,
            rate_func=linear,
        )
        # Apex marker and height label
        apex_point = moving_particle2.get_center()
        apex_marker = Dot(apex_point, color=RED, radius=0.08)
        H = v0y ** 2 / (2 * g)
        height_label = Text(f"H ≈ {H:.2f} m", font_size=24).next_to(apex_marker, UP)
        self.play(FadeIn(apex_marker), FadeIn(height_label), run_time=1.0)

        self.wait(1.0)  # pause at apex

        # Animate to landing (t_flight)
        self.play(
            time_tracker2.animate.set_value(t_flight),
            run_time=3.0,
            rate_func=linear,
        )
        # Range marker and label
        landing_point = moving_particle2.get_center()
        range_marker = Dot(landing_point, color=ORANGE, radius=0.08)
        R = v0 ** 2 * np.sin(2 * theta) / g
        range_label = Text(f"R ≈ {R:.2f} m", font_size=24).next_to(range_marker, DOWN)
        self.play(FadeIn(range_marker), FadeIn(range_label), run_time=1.0)

        self.wait(5.3)  # total 15.3 s

        # End of presentation
        self.play(FadeOut(header), FadeOut(status4), run_time=1.0)