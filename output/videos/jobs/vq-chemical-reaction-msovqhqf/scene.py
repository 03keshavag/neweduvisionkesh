...
class AutoTeach(Scene):
    def construct(self):
        ...
        # Initial empty status bar
        status = LayoutManager.create_status_bar("")
        # -------------------------------------------------
        # ------------------- SCENE 1 --------------------
        # -------------------------------------------------
        # (no previous scene to clear)
        # Create methane molecule
        def make_methane(center):
            C = MoleculeVisualizer.make_atom("C", BLUE, 0.3)
            H1 = MoleculeVisualizer.make_atom("H", LIGHT_GRAY, 0.2)
            H2 = MoleculeVisualizer.make_atom("H", LIGHT_GRAY, 0.2)
            H3 = MoleculeVisualizer.make_atom("H", LIGHT_GRAY, 0.2)
            H4 = MoleculeVisualizer.make_atom("H", LIGHT_GRAY, 0.2)
            C.move_to(center)
            offset = 0.6
            H1.move_to(center + np.array([ offset, 0.0, 0.0]))
            H2.move_to(center + np.array([-offset, 0.0, 0.0]))
            H3.move_to(center + np.array([0.0,  offset, 0.0]))
            H4.move_to(center + np.array([0.0, -offset, 0.0]))
            ...
            return VGroup(C, H1, H2, H3, H4, b1, b2, b3, b4)

        def make_oxygen(center):
            O1 = MoleculeVisualizer.make_atom("O", RED, 0.25)
            O2 = MoleculeVisualizer.make_atom("O", RED, 0.25)
            O1.move_to(center + np.array([-0.3, 0.0, 0.0]))
            O2.move_to(center + np.array([ 0.3, 0.0, 0.0]))
            bond = MoleculeVisualizer.make_bond(O1, O2)
            return VGroup(O1, O2, bond)

        methane = make_methane(np.array([-3.0, 0.0, 0.0]))
        oxygen1 = make_oxygen(np.array([2.0, 0.8, 0.0]))
        oxygen2 = make_oxygen(np.array([2.0, -0.8, 0.0]))
        reaction_arrow = Arrow(
            start=methane.get_right() + np.array([0.2, 0.0, 0.0]),
            end=oxygen1.get_left() + np.array([-0.2, 0.0, 0.0]),
            buff=0.0,
            color=YELLOW
        )
        # Status update for scene 1
        status1 = LayoutManager.create_status_bar(
            "Methane and two oxygen molecules ready."
        )
        self.play(ReplacementTransform(status, status1), run_time=0.5)
        # Animate appearance
        self.play(FadeIn(methane), run_time=1.5)
        self.play(FadeIn(oxygen1), run_time=1.5)
        self.play(FadeIn(oxygen2), run_time=1.5)
        self.play(FadeIn(reaction_arrow), run_time=1.0)
        self.wait(3.4)  # total 9.4 s

        # -------------------------------------------------
        # ------------------- SCENE 2 --------------------
        # -------------------------------------------------
        self.clear_stage(preserve=header)
        # Re‑create molecules for continuity
        methane = make_methane(np.array([-3.0, 0.0, 0.0]))
        oxygen1 = make_oxygen(np.array([2.0, 0.8, 0.0]))
        oxygen2 = make_oxygen(np.array([2.0, -0.8, 0.0]))
        self.add(methane, oxygen1, oxygen2)
        # New status
        status2 = LayoutManager.create_status_bar(
            "Atoms vibrate, O₂ rushes toward CH₄, bonds stretch."
        )
        self.play(ReplacementTransform(status1, status2), run_time=1.0)
        # Vibration (simple left‑right shift)
        all_atoms = VGroup(*[m for m in methane if isinstance(m, Mobject)],
                           *[o for o in oxygen1 if isinstance(o, Mobject)],
                           *[o for o in oxygen2 if isinstance(o, Mobject)])
        self.play(
            *[atom.animate.shift(np.array([0.05, 0.0, 0.0])) for atom in all_atoms],
            run_time=1.0
        )
        self.play(
            *[atom.animate.shift(np.array([-0.05, 0.0, 0.0])) for atom in all_atoms],
        ...
        # Move O₂ molecules toward CH₄
        self.play(
            oxygen1.animate.shift(np.array([-2.5, 0.0, 0.0])),
            oxygen2.animate.shift(np.array([-2.5, 0.0, 0.0])),
            run_time=2.0
        )
        # Flash breaking bonds in red
        bonds = VGroup(*[m for m in methane if isinstance(m, Mobject) and m.get_color() != WHITE],
                       *[m for m in oxygen1 if isinstance(m, Mobject) and m.get_color() != WHITE],
                       *[m for m in oxygen2 if isinstance(m, Mobject) and m.get_color() != WHITE])
        self.play(Indicate(bonds, color=RED), run_time=1.5)
        self.wait(4.2)  # total 10.7 s
        ...
        status3 = LayoutManager.create_status_bar(
            "Forming CO₂ and H₂O, releasing energy."
        )
        ...
        def make_co2(center):
            ...
            O2 = MoleculeVisualizer.make_atom("O", RED, 0.25)
            C.move_to(center)
            O1.move_to(center + np.array([-0.5, 0.0, 0.0]))
            O2.move_to(center + np.array([ 0.5, 0.0, 0.0]))
            b1 = MoleculeVisualizer.make_bond(C, O1)
            b2 = MoleculeVisualizer.make_bond(C, O2)
            return VGroup(C, O1, O2, b1, b2)