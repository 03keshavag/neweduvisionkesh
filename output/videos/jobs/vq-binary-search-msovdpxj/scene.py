...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ---------- HEADER (persistent across scenes) ----------
        header = LayoutManager.create_header("Binary Search", "Search Algorithm")
        self.play(FadeIn(header, run_time=1.0))

        # ---------- SCENE 1 ----------
        # Setup array, pointers, and target card
        array_vals = [3, 8, 12, 17, 23, 31, 42]
        array_vis = ArrayVisualizer(
            array_vals,
            box_size=0.8,
            color=BLUE_D,
            font_size=22,
        ...
        low_ptr = Text("LOW", font_size=24, color=YELLOW).next_to(array_vis[0], UP, buff=0.25)
        high_ptr = Text("HIGH", font_size=24, color=YELLOW).next_to(array_vis[6], UP, buff=0.25)
        target_card = LayoutManager.create_equation_card("Target: 23", "Target")
        # Position target card in the top‑right corner of the main stage
        target_card.move_to(np.array([5.5, 2.5, 0]))

        # Animate scene‑1 elements
        self.play(FadeIn(array_vis, run_time=1.5))
        self.play(FadeIn(low_ptr, run_time=0.8))
        self.play(FadeIn(high_ptr, run_time=0.8))
        self.play(FadeIn(target_card, run_time=0.8))
        # Wait for the remainder of the narration
        self.wait(6.4)  # 1.0+1.5+0.8+0.8+0.8 = 4.9 → 11.3‑4.9 = 6.4

        # ---------- SCENE 2 ----------
        # Clear previous visual elements but keep header
        self.clear_stage(preserve=header)

        # Re‑create array and pointers for the new scene
        array_vis = ArrayVisualizer(
            array_vals,
            box_size=0.8,
            color=BLUE_D,
        ...
        low_ptr = Text("LOW", font_size=24, color=YELLOW).next_to(array_vis[0], UP, buff=0.25)
        high_ptr = Text("HIGH", font_size=24, color=YELLOW).next_to(array_vis[6], UP, buff=0.25)
        mid_ptr = Text("MID", font_size=24, color=ORANGE).next_to(array_vis[3], UP, buff=0.25)

        status1 = LayoutManager.create_status_bar(
            "First, compute MID = (0+6)//2 = 3 → value 17, LOW moves to 4"
        )
        self.play(FadeIn(array_vis, run_time=1.2))
        self.play(FadeIn(low_ptr, run_time=0.6))
        self.play(FadeIn(high_ptr, run_time=0.6))
        self.play(FadeIn(mid_ptr, run_time=0.6))
        self.play(FadeIn(status1, run_time=0.8))
        self.wait(1.0)

        # Move LOW pointer to index 4
        self.play(
            low_ptr.animate.next_to(array_vis[4], UP, buff=0.25),
            run_time=1.0,
        )
        # Fade out the left half (indices 0‑3)
        left_half = VGroup(*array_vis[:4])
        self.play(FadeOut(left_half, run_time=1.2))

        # Update status bar
        status2 = LayoutManager.create_status_bar(
            "LOW = 4, left side dimmed"
        )
        self.play(
            ReplacementTransform(status1, status2),
            run_time=1.0,
        )
        self.wait(6.9)  # Total = 14.1 seconds

        # ---------- SCENE 3 ----------
        self.clear_stage(preserve=header)

        # Re‑create only the remaining visible part of the array (indices 4‑6)
        remaining_vals = array_vals[4:7]  # [23, 31, 42]
        array_vis = ArrayVisualizer(
            remaining_vals,
            box_size=0.8,
            color=BLUE_D,
        ...
        # Adjust indices to match original positions for positioning
        for i, box in enumerate(array_vis):
            box.shift(np.array([i * 1.2, 0, 0]))  # simple horizontal layout

        low_ptr = Text("LOW", font_size=24, color=YELLOW).next_to(array_vis[0], UP, buff=0.25)   # index 4
        high_ptr = Text("HIGH", font_size=24, color=YELLOW).next_to(array_vis[2], UP, buff=0.25)  # index 6
        mid_ptr = Text("MID", font_size=24, color=ORANGE).next_to(array_vis[2], UP, buff=0.25)   # index 5 (value 31)

        status3 = LayoutManager.create_status_bar(
            "MID = (4+6)//2 = 5 → value 31, HIGH moves to 4"
        )
        self.play(FadeIn(array_vis, run_time=1.2))
        self.play(FadeIn(low_ptr, run_time=0.6))
        self.play(FadeIn(high_ptr, run_time=0.6))
        self.play(FadeIn(mid_ptr, run_time=0.6))
        self.play(FadeIn(status3, run_time=0.8))
        self.wait(1.0)

        # Move HIGH pointer down to index 4 (which is array_vis[0] in this reduced view)
        self.play(
            high_ptr.animate.next_to(array_vis[0], UP, buff=0.25),
            run_time=1.0,
        )
        # Fade out the right side (indices 5‑6 of original array → array_vis[1] and [2])
        right_side = VGroup(array_vis[1], array_vis[2])
        self.play(FadeOut(right_side, run_time=1.2))

        # Update status bar
        status4 = LayoutManager.create_status_bar(
            "HIGH = 4, right side dimmed"
        )
        self.play(
            ReplacementTransform(status3, status4),
            run_time=1.0,
        )
        self.wait(9.6)  # Total = 14.4 seconds

        # ---------- SCENE 4 ----------
        self.clear_stage(preserve=header)

        # Show the final single element (index 4)
        final_elem = ArrayVisualizer(
            [23],
            box_size=0.8,
            color=BLUE_D,
            font_size=22,
        ...
        final_elem.move_to(np.array([0, 0, 0]))
        mid_ptr = Text("MID", font_size=24, color=ORANGE).next_to(final_elem[0], UP, buff=0.25)

        status5 = LayoutManager.create_status_bar(
            "MID = 4 → value 23 matches target"
        )
        self.play(FadeIn(final_elem, run_time=1.2))
        self.play(FadeIn(mid_ptr, run_time=0.6))
        self.play(FadeIn(status5, run_time=0.8))
        self.wait(0.8)

        # Highlight the found element in GREEN
        self.play(
            final_elem[0].animate.set_fill(GREEN, opacity=1.0),
            run_time=1.0,
        )
        # Show result text
        result_text = Text("Found at index 4", font_size=28, color=GREEN)
        result_text.next_to(final_elem[0], DOWN, buff=0.5)
        self.play(FadeIn(result_text, run_time=1.0))

        # Update status bar to final message
        status6 = LayoutManager.create_status_bar(
            "Search complete – target located"
        )
        self.play(
            ReplacementTransform(status5, status6),
            run_time=1.0,
        )
        self.wait(9.1)  # Total = 13.5 seconds