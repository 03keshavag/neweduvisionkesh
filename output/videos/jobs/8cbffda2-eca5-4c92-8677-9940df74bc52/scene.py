...
class AutoTeach(Scene):
    def construct(self):
        ...
        header = LayoutManager.create_header("Pigeonhole Principle", "Explanation")
        self.add(header)

        # ==================== SCENE 1 ====================
        # Create 5 holes
        hole_positions = [np.array([x, -1, 0]) for x in [-2, -1, 0, 1, 2]]
        holes = VGroup()
        hole_labels = VGroup()
        for i, pos in enumerate(hole_positions, start=1):
            box = Square(side_length=0.8, color=WHITE).move_to(pos)
            label = Text(f"Hole {i}", font_size=24).next_to(box, DOWN)
            holes.add(box)
            hole_labels.add(label)
        # Create 7 pigeons
        pigeon_names = ["A","B","C","D","E","F","G"]
        pigeon_positions = [np.array([x, 2, 0]) for x in [-2.5, -1.5, -0.5, 0.5, 1.5, -2.5, -1.5]]
        pigeons = VGroup()
        pigeon_labels = VGroup()
        for name, pos in zip(pigeon_names, pigeon_positions):
            dot = Dot(point=pos, radius=0.15, color=YELLOW)
            label = Text(f"Pigeon {name}", font_size=20).next_to(dot, UP)
            pigeons.add(dot)
            pigeon_labels.add(label)
        # Animate scene 1
        self.play(FadeIn(holes, run_time=2.0))
        self.play(FadeIn(hole_labels, run_time=0.5))
        self.wait(0.5)
        self.play(FadeIn(pigeons, run_time=2.0))
        self.play(FadeIn(pigeon_labels, run_time=0.5))
        self.wait(1.0)  # total 7.2 seconds

        # ==================== SCENE 2 ====================
        self.clear_stage(preserve=header)
        # Recreate holes and pigeons
        self.add(holes, hole_labels, pigeons, pigeon_labels)
        # Mapping pigeons to holes (ensure at least one double)
        mapping = {
            "A": 0,  # Hole 1
            "B": 1,  # Hole 2
            "C": 2,  # Hole 3
            "D": 3,  # Hole 4
            "E": 4,  # Hole 5
            "F": 1,  # Hole 2 again (double)
            "G": 2,  # Hole 3 again (double)
        }
        # Animate drops
        for i, name in enumerate(pigeon_names):
            target_hole = holes[mapping[name]]
            target_pos = target_hole.get_center() + np.array([0, 0, 0])
            self.play(pigeons[i].animate.move_to(target_pos), run_time=0.5)
            if i < len(pigeon_names) - 1:
                self.wait(0.2)
        # Highlight a hole that received a second pigeon (choose Hole 2)
        highlight = Rectangle(width=0.9, height=0.9, color=RED, stroke_width=4).move_to(holes[1].get_center())
        self.play(FadeIn(highlight), run_time=0.6)
        self.wait(1.4)  # total 6.7 seconds

        # ==================== SCENE 3 ====================
        self.clear_stage(preserve=header)
        # Recreate holes and pigeons at original positions
        self.add(holes, hole_labels, pigeons, pigeon_labels)
        # Create arrows (relationship vectors)
        arrows = VGroup()
        for i, name in enumerate(pigeon_names):
            start = pigeons[i].get_center()
            end = holes[mapping[name]].get_center()
            line = Line(start, end, color=GREEN)
            arrows.add(line)
        self.play(FadeIn(arrows, run_time=2.0))
        self.wait(0.5)
        # Replace arrows with formula
        formula = Text("n > m ⇒ ∃ hole with ≥ 2 items", font_size=36, color=WHITE)
        self.play(ReplacementTransform(arrows, formula), run_time=2.0)
        self.wait(0.5)
        self.play(formula.animate.scale(1.2), run_time=1.0)
        self.wait(0.5)
        self.play(formula.animate.set_color(YELLOW), run_time=0.5)
        self.wait(0.5)
        self.play(formula.animate.set_opacity(1.0), run_time=0.5)
        self.wait(3.7)  # total 11.2 seconds

        # ==================== SCENE 4 ====================
        self.clear_stage(preserve=header)
        # Summary badge
        badge_bg = Rectangle(width=6, height=2, stroke_width=4, stroke_color=BLUE, fill_opacity=0.2, fill_color=BLACK)
        badge_text = Text("If n > m then some box contains ≥ 2 items", font_size=30, color=WHITE)
        badge_text.move_to(badge_bg.get_center())
        badge = VGroup(badge_bg, badge_text)
        self.play(FadeIn(badge, run_time=2.0))
        self.wait(0.5)
        self.play(badge.animate.set_stroke(YELLOW, width=6), run_time=1.5)
        self.wait(0.5)
        self.play(badge.animate.scale(1.1), run_time=0.8)
        self.wait(0.5)
        self.play(badge.animate.scale(0.95), run_time=0.8)
        self.wait(1.9)  # total 9.5 seconds