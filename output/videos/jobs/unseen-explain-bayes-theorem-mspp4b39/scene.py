...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ----- Header (present throughout) -----
        header = LayoutManager.create_header("Bayes' Theorem")
        self.play(FadeIn(header, run_time=1.0))
        self.wait(0.5)

        # ==============================================================
        # Scene 1 – Setup & Frame of Reference (10.7 s)
        # ==============================================================

        # Prior circle
        prior_circle = Circle(radius=2.0, color=BLUE_D).move_to(np.array([-1.2, 0.0, 0.0]))
        prior_label = Text("Prior\n0.6", font_size=36, color=WHITE).move_to(prior_circle.get_center())

        # Likelihood circle
        like_circle = Circle(radius=2.0, color=GREEN_D).move_to(np.array([1.2, 0.0, 0.0]))
        like_label = Text("Likelihood\n0.7", font_size=36, color=WHITE).move_to(like_circle.get_center())

        circles_group = VGroup(prior_circle, like_circle, prior_label, like_label)
        LayoutManager.safe_scale(circles_group)

        # Animate appearance
        self.play(FadeIn(prior_circle, run_time=1.0))
        self.play(FadeIn(like_circle, run_time=1.0))
        self.play(FadeIn(prior_label, run_time=0.5), FadeIn(like_label, run_time=0.5))
        self.wait(0.5)

        # Fill remaining time for this scene
        self.wait(7.4)   # 1+1+0.5+0.5+0.5+7.4 = 10.7 s

        # ==============================================================
        # Scene 2 – Animate Core Transformation (9.2 s)
        # ==============================================================

        LayoutManager.clear_stage(self, preserve=header)

        # Re‑create circles and labels
        prior_circle = Circle(radius=2.0, color=BLUE_D).move_to(np.array([-1.2, 0.0, 0.0]))
        prior_label = Text("Prior\n0.6", font_size=36, color=WHITE).move_to(prior_circle.get_center())
        like_circle = Circle(radius=2.0, color=GREEN_D).move_to(np.array([1.2, 0.0, 0.0]))
        like_label = Text("Likelihood\n0.7", font_size=36, color=WHITE).move_to(like_circle.get_center())
        ...
        self.play(FadeIn(circles_group, run_time=2.0))
        self.wait(0.3)

        # Joint label above overlap
        joint_label = Text("Joint = 0.6×0.7 = 0.42", font_size=34, color=YELLOW)
        joint_label.move_to(np.array([0.0, 2.2, 0.0]))
        self.play(FadeIn(joint_label, run_time=1.0))
        self.wait(0.3)

        # Arrow from Prior label to Joint label (multiplication)
        arrow1 = Arrow(start=prior_label.get_center(), end=joint_label.get_center(),
                       buff=0.2, color=YELLOW)
        self.play(FadeIn(arrow1, run_time=0.8))
        self.wait(0.4)
        self.play(FadeOut(arrow1, run_time=0.5))

        # Arrow from Likelihood label to Joint label
        arrow2 = Arrow(start=like_label.get_center(), end=joint_label.get_center(),
                       buff=0.2, color=YELLOW)
        self.play(FadeIn(arrow2, run_time=0.8))
        self.wait(0.5)
        self.play(FadeOut(arrow2, run_time=0.5))

        # Fill remaining time for this scene
        self.wait(2.7)   # total = 9.2 s

        # ==============================================================
        # Scene 3 – Highlight Key Inflection Point (9.9 s)
        # ==============================================================
        ...
        # Joint label (same position as before)
        joint_label = Text("Joint = 0.6×0.7 = 0.42", font_size=34, color=YELLOW)
        joint_label.move_to(np.array([0.0, 2.2, 0.0]))
        self.play(FadeIn(joint_label, run_time=1.0))
        self.wait(0.3)

        # Denominator label (total evidence)
        denom_label = Text("P(Evidence)=0.5", font_size=34, color=ORANGE)
        denom_label.move_to(np.array([0.0, -2.2, 0.0]))
        self.play(FadeIn(denom_label, run_time=1.0))
        self.wait(0.5)

        # Transform Joint into Posterior
        posterior_label = Text("Posterior = 0.42/0.5 = 0.84", font_size=34, color=GREEN)
        posterior_label.move_to(joint_label.get_center())
        self.play(ReplacementTransform(joint_label, posterior_label, run_time=1.2))
        self.wait(4.2)   # total = 9.9 s

        # ==============================================================
        # Scene 4 – Present Summary (14.6 s)
        # ==============================================================

        LayoutManager.clear_stage(self, preserve=header)

        # Badge background
        badge_bg = RoundedRectangle(width=9.0, height=2.5, corner_radius=0.3,
                                    fill_color=BLACK, fill_opacity=0.8, stroke_color=WHITE)
        badge_bg.move_to(np.array([0.0, 0.0, 0.0]))

        # Formula text
        formula_text = Text("P(H|E)=P(E|H)·P(H)/P(E)", font_size=36, color=WHITE)
        formula_text.move_to(badge_bg.get_center())

        formula_badge = VGroup(badge_bg, formula_text)
        self.play(FadeIn(formula_badge, run_time=2.0))
        self.wait(0.5)

        # Highlight components sequentially
        components = ["P(H|E)", "P(E|H)", "P(H)", "P(E)"]
        highlights = []
        for comp in components:
            comp_mob = Text(comp, font_size=36, color=YELLOW)
            comp_mob.move_to(formula_text.get_center())
            # Find bounding box of component within formula_text (approximate using surrounding rectangle)
            rect = SurroundingRectangle(comp_mob, color=YELLOW, buff=0.15)
