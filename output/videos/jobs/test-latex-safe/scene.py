...
class AutoTeach(Scene):
    def construct(self):
        # --------------------------------------------------------------------
        # Common Header (kept throughout)
        # --------------------------------------------------------------------
        header = LayoutManager.create_header(
            "Exponential Growth in BFS", "Linking visited nodes to eⁿ"
        )
        self.play(FadeIn(header), run_time=1.0)

        # --------------------------------------------------------------------
        # Scene 1 – Graph Setup (duration 10.1 s)
        # --------------------------------------------------------------------
        status1 = LayoutManager.create_status_bar("Scene 1: Graph Setup")
        self.play(FadeIn(status1), run_time=0.8)

        # node positions (3‑D vectors)
        pos = {
            "A": np.array([0, 2, 0]),
            "B": np.array([-2, 0, 0]),
            "C": np.array([2, 0, 0]),
        ...
        graph = GraphVisualizer(pos, edges).get_group()
        self.play(Create(graph), run_time=3.0)

        start_hl = Circle(radius=0.4, color=YELLOW).move_to(pos["A"])
        self.play(FadeIn(start_hl), run_time=0.8)

        self.wait(4.5)  # 1.0 + 0.8 + 3.0 + 0.8 + 4.5 = 10.1 s

        # --------------------------------------------------------------------
        # Scene 2 – First Expansion – Queue Doubling (duration 9.7 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)
        status2 = LayoutManager.create_status_bar(
            "Scene 2: First Expansion – Queue Doubling"
        )
        self.play(FadeIn(status2), run_time=0.8)

        # queue visual (bottom)
        queue_txt = Text("Queue: []", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(FadeIn(queue_txt), run_time=0.8)

        # enqueue A
        a_dot = Dot(pos["A"], radius=0.15, color=YELLOW)
        self.play(FadeIn(a_dot), run_time=0.6)
        self.play(
            a_dot.animate.move_to(queue_txt.get_left() + np.array([1, 0, 0])),
            run_time=0.8,
        )
        q1 = Text("Queue: [A]", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(ReplacementTransform(queue_txt, q1), run_time=0.8)

        # visit A (turn green)
        self.play(start_hl.animate.set_color(GREEN), run_time=0.8)

        # enqueue B and C
        b_dot = Dot(pos["B"], radius=0.15, color=YELLOW)
        c_dot = Dot(pos["C"], radius=0.15, color=YELLOW)
        self.play(FadeIn(b_dot), FadeIn(c_dot), run_time=0.8)
        self.play(
            b_dot.animate.move_to(q1.get_left() + np.array([1.5, 0, 0])),
            c_dot.animate.move_to(q1.get_left() + np.array([2.0, 0, 0])),
            run_time=1.0,
        )
        q2 = Text("Queue: [B, C]", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(ReplacementTransform(q1, q2), run_time=0.8)

        self.wait(2.5)  # total 9.7 s

        # --------------------------------------------------------------------
        # Scene 3 – Continued Growth – Approximate eˣ (duration 12.8 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)
        status3 = LayoutManager.create_status_bar(
            "Scene 3: Continued Growth – Approximate eˣ"
        )
        ...
        # mark A as visited (green)
        a_vis = Dot(pos["A"], radius=0.15, color=GREEN)
        self.play(FadeIn(a_vis), run_time=0.6)

        # initial queue display [B, C]
        queue3 = Text("Queue: [B, C]", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(FadeIn(queue3), run_time=0.6)

        # ---- Visit B ----
        b_vis = Dot(pos["B"], radius=0.15, color=GREEN)
        self.play(FadeIn(b_vis), run_time=0.6)

        # enqueue D and E, update queue
        d_vis = Dot(pos["D"], radius=0.15, color=YELLOW)
        e_vis = Dot(pos["E"], radius=0.15, color=YELLOW)
        self.play(FadeIn(d_vis), FadeIn(e_vis), run_time=0.6)
        q_after_b = Text("Queue: [C, D, E]", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(ReplacementTransform(queue3, q_after_b), run_time=0.6)

        # ---- Visit C ----
        c_vis = Dot(pos["C"], radius=0.15, color=GREEN)
        self.play(FadeIn(c_vis), run_time=0.6)

        # enqueue F, update queue
        f_vis = Dot(pos["F"], radius=0.15, color=YELLOW)
        self.play(FadeIn(f_vis), run_time=0.6)
        q_final = Text("Queue: [D, E, F]", font_size=24).to_edge(DOWN, buff=0.5)
        self.play(ReplacementTransform(q_after_b, q_final), run_time=0.6)

        # visited count overlay
        visited_cnt = Text("Visited nodes: 5", font_size=28, color=WHITE).to_corner(
            UL
        )
        self.play(FadeIn(visited_cnt), run_time=0.8)

        self.wait(4.4)  # total 12.8 s

        # --------------------------------------------------------------------
        # Scene 4 – Link to eˣ Curve (duration 10.1 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)
        status4 = LayoutManager.create_status_bar(
            "Scene 4: Link to eˣ Curve"
        )
        self.play(FadeIn(status4), run_time=0.8)
        ...
        # plot e^x (approximate using lambda)
        exp_graph = axes.plot(lambda x: np.exp(x), x_range=[0, 3], color=RED)
        self.play(Create(exp_graph), run_time=2.0)

        # bar chart of visited counts per BFS step
        # steps: 0→1, 1→3, 2→6 (use simple rectangles)
        bars = VGroup()
        step_vals = [1, 3, 6]
        for i, val in enumerate(step_vals):
            bar = Rectangle(
                width=0.4,
                height=axes.c2p(0, val)[1] - axes.c2p(0, 0)[1],
                fill_color=BLUE,
                fill_opacity=0.6,
                stroke_width=0,
            ).move_to(
        ...
        self.play(FadeIn(bars), run_time=2.0)

        self.wait(3.3)  # total 10.1 s