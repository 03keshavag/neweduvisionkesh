...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ---------- Header (persistent) ----------
        header = LayoutManager.create_header("Dijkstra Algorithm", "Shortest Paths")
        ...
        status1 = LayoutManager.create_status_bar(
            "Setup weighted graph with nodes A‑F and edges."
        )
        self.play(FadeIn(status1), run_time=0.8)
        self.wait(0.5)

        graph_viz = GraphVisualizer(pos, edges).get_group()
        self.play(FadeIn(graph_viz), run_time=2.0)
        self.wait(0.5)

        # Highlight start node A in yellow
        node_A = graph_viz.submobjects[0]  # assume first node corresponds to A
        self.play(node_A.animate.set_fill(YELLOW, opacity=0.8), run_time=0.8)
        self.wait(0.5)

        # Empty queue display at bottom
        queue_display = VGroup()
        queue_display.move_to(np.array([0, -3.0, 0]))
        self.play(FadeIn(queue_display), run_time=0.8)