...
class AutoTeach(Scene):
    def construct(self):
        ...
        subtitle = Text("Using a stack", font_size=24, color=WHITE)
        ...
        for name, pos in positions.items():
            ...
            label = Text(name, font_size=24, color=WHITE).move_to(pos)
            node = VGroup(circ, label)
            nodes[name] = node
            node_group.add(node)

        # create edges and store in a dict for easy access
        edge_lines = {}
        edge_group = VGroup()
        for a, b in edges:
            line = Line(positions[a], positions[b], color=WHITE)
            line.set_z_index(-1)  # behind nodes
            edge_group.add(line)
            edge_lines[(a, b)] = line
            edge_lines[(b, a)] = line  # allow reverse lookup

        graph = VGroup(edge_group, node_group)
        self.play(Create(graph))
        self.wait(0.7)

        # ---------- Stack Display ----------
        stack_origin = np.array([0, -2.5, 0])
        stack_box_width = 2.0
        stack_box_height = 0.6
        stack_entries = []

        stack_label = Text("Stack (top → bottom)", font_size=24, color=WHITE)
        stack_label.next_to(stack_origin, UP, buff=0.2)
        self.play(FadeIn(stack_label))
        self.wait(0.3)

        def push_to_stack(label_text):
            box = Rectangle(
            ...
            txt = Text(label_text, font_size=24, color=WHITE)
            txt.move_to(box.get_center())
            box.add(txt)
            if stack_entries:
                box.next_to(stack_entries[-1], DOWN, buff=0.0)
            else:
                box.move_to(stack_origin)
            stack_entries.append(box)
            self.play(FadeIn(box))

        def pop_from_stack():
            if not stack_entries:
                return None
            top = stack_entries.pop()
            self.play(FadeOut(top))
            return top

        # ---------- Status Bar ----------
        status = Text("", font_size=20, color=WHITE).to_edge(DOWN, buff=0.5)
        self.play(FadeIn(status))
        self.wait(0.3)

        # ---------- Scene 1: Setup ----------
        self.play(status.animate.set_text("Setup graph and empty stack"))
        self.wait(1)

        # ---------- Scene 2: Push start node ----------
        self.play(status.animate.set_text("Push start node A, pop and visit"))
        push_to_stack("A")
        self.wait(0.5)
        pop_from_stack()
        # Mark A as visited
        node_A = nodes["A"]
        self.play(node_A[0].animate.set_fill(GREEN, opacity=0.8))
        badge_A = Text("✔", font_size=24, color=GREEN).next_to(node_A, UP, buff=0.1)
        self.play(FadeIn(badge_A))
        self.wait(0.7)

        # ---------- Scene 3: Discover neighbors ----------
        self.play(status.animate.set_text("Discover neighbors C and B, push C then B"))
        edge_AC = edge_lines[("A", "C")]
        edge_AB = edge_lines[("A", "B")]
        self.play(edge_AC.animate.set_color(YELLOW), edge_AB.animate.set_color(YELLOW))
        self.wait(0.5)
        push_to_stack("C")
        self.wait(0.3)
        push_to_stack("B")
        self.wait(0.5)
        self.play(edge_AC.animate.set_color(WHITE), edge_AB.animate.set_color(WHITE))
        self.wait(0.5)

        # ---------- Scene 4: Full traversal ----------
        self.play(status.animate.set_text("DFS traversal using stack"))

        def visit_node(name):
            node = nodes[name]
            self.play(node[0].animate.set_fill(GREEN, opacity=0.8))
            badge = Text("✔", font_size=24, color=GREEN).next_to(node, UP, buff=0.1)
            self.play(FadeIn(badge))
            return badge

        # Pop B, visit, push D then E
        pop_from_stack()  # B
        badge_B = visit_node("B")
        push_to_stack("D")
        self.wait(0.3)
        push_to_stack("E")
        self.wait(0.5)
        ...
        # Pop D, visit
        pop_from_stack()  # D
        badge_D = visit_node("D")
        self.wait(0.5)

        # Pop E, visit
        pop_from_stack()  # E
        badge_E = visit_node("E")
        self.wait(0.5)

        # Pop C, visit, push F
        pop_from_stack()  # C
        badge_C = visit_node("C")
        push_to_stack("F")
        self.wait(0.5)

        # Pop F, visit
        pop_from_stack()  # F
        badge_F = visit_node("F")
        self.wait(0.7)

        # Final visited order display
        order_text = Text(
            "DFS order: A → B → D → E → C → F",
            font_size=28,
            color=YELLOW,
        ).next_to(status, DOWN, buff=0.4)
