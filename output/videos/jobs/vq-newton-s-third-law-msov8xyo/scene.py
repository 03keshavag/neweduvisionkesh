...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ==================== SCENE 1 ====================
        # Clear any previous content (none at first) but keep header
        self.clear_stage(preserve=header)

        # Status bar for scene 1
        status1 = LayoutManager.create_status_bar(
            "Place 2 kg block, draw 10 N force right, friction left."
        )
        self.play(FadeIn(status1), run_time=1.0)

        # Kinematics canvas (ground + axes)
        axes, ground = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        self.play(FadeIn(axes), FadeIn(ground), run_time=1.0)

        # Block (square) and its label
        block = Square(side_length=0.8, color=BLUE_D).move_to(np.array([-2, 0, 0]))
        block_label = Text("m = 2 kg", font_size=24).next_to(block, UP, buff=0.25)
        self.play(FadeIn(block), FadeIn(block_label), run_time=1.0)

        # Force arrow (rightward) and label
        force_start = block.get_right()
        force_end = force_start + np.array([2, 0, 0])
        force_arrow = Arrow(force_start, force_end, buff=0, color=YELLOW)
        force_label = Text("F = 10 N", font_size=24).next_to(force_arrow, UP, buff=0.25)
        self.play(GrowArrow(force_arrow), FadeIn(force_label), run_time=1.0)

        # Friction arrow (leftward) and label
        friction_start = block.get_left()
        friction_end = friction_start + np.array([-1, 0, 0])
        friction_arrow = Arrow(friction_start, friction_end, buff=0, color=RED)
        friction_label = Text("f", font_size=24).next_to(friction_arrow, DOWN, buff=0.25)
        self.play(GrowArrow(friction_arrow), FadeIn(friction_label), run_time=1.0)

        # Wait to match total duration 10.5 s (so far 5 s of play)
        self.wait(5.5)

        # ==================== SCENE 2 ====================
        # Clean stage but keep header
        self.clear_stage(preserve=header)

        # Update status bar
        status2 = LayoutManager.create_status_bar(
            "Force pushes block, acceleration 5 m/s², surface reaction –10 N."
        )
        self.play(ReplacementTransform(status1, status2), run_time=1.0)

        # Re‑create canvas
        axes2, ground2 = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        self.play(FadeIn(axes2), FadeIn(ground2), run_time=1.0)

        # Re‑create block and label
        block2 = Square(side_length=0.8, color=BLUE_D).move_to(np.array([-2, 0, 0]))
        block2_label = Text("m = 2 kg", font_size=24).next_to(block2, UP, buff=0.25)
        self.play(FadeIn(block2), FadeIn(block2_label), run_time=1.0)

        # Force arrow (rightward)
        f_start = block2.get_right()
        f_end = f_start + np.array([2, 0, 0])
        force_arrow2 = Arrow(f_start, f_end, buff=0, color=YELLOW)
        f_label2 = Text("F = 10 N", font_size=24).next_to(force_arrow2, UP, buff=0.25)
        self.play(GrowArrow(force_arrow2), FadeIn(f_label2), run_time=0.5)

        # Acceleration vector (rightward, shorter)
        a_start = block2.get_center()
        a_end = a_start + np.array([1, 0, 0])
        accel_arrow = Arrow(a_start, a_end, buff=0, color=GREEN)
        accel_label = Text("a = 5 m/s²", font_size=24).next_to(accel_arrow, UP, buff=0.25)
        self.play(GrowArrow(accel_arrow), FadeIn(accel_label), run_time=0.8)

        # Reaction arrow from surface (leftward)
        r_start = block2.get_left()
        r_end = r_start + np.array([-2, 0, 0])
        reaction_arrow = Arrow(r_start, r_end, buff=0, color=RED)
        reaction_label = Text("R = –10 N", font_size=24).next_to(reaction_arrow, DOWN, buff=0.25)
        self.play(GrowArrow(reaction_arrow), FadeIn(reaction_label), run_time=0.8)

        # Move block to illustrate acceleration
        self.play(
            block2.animate.shift(np.array([2, 0, 0])),
            run_time=2.0
        )

        # Wait to reach total 11.2 s (so far 1+1+1+0.5+0.8+0.8+2 = 7.1 s)
        self.wait(4.1)
        ...
        status3 = LayoutManager.create_status_bar(
            "Equation F = m·a, numbers substituted, reaction matches force."
        )
        self.play(ReplacementTransform(status2, status3), run_time=1.0)

        # Equation card
        equation = LayoutManager.create_equation_card(
            "F = m·a   (10 N = 2 kg·5 m/s²)",
            "Newton’s 2nd Law"
        )
        self.play(FadeIn(equation), run_time=1.0)

        # Re‑create minimal visual: block, force arrow, reaction arrow
        axes3, ground3 = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        self.play(FadeIn(axes3), FadeIn(ground3), run_time=0.5)

        block3 = Square(side_length=0.8, color=BLUE_D).move_to(np.array([-2, 0, 0]))
        block3_label = Text("m = 2 kg", font_size=24).next_to(block3, UP, buff=0.25)
        self.play(FadeIn(block3), FadeIn(block3_label), run_time=0.5)

        # Force arrow
        f3_start = block3.get_right()
        f3_end = f3_start + np.array([2, 0, 0])
        force_arrow3 = Arrow(f3_start, f3_end, buff=0, color=YELLOW)
        f3_label = Text("F = 10 N", font_size=24).next_to(force_arrow3, UP, buff=0.25)
        self.play(GrowArrow(force_arrow3), FadeIn(f3_label), run_time=0.5)

        # Reaction arrow
        r3_start = block3.get_left()
        r3_end = r3_start + np.array([-2, 0, 0])
        reaction_arrow3 = Arrow(r3_start, r3_end, buff=0, color=RED)
        r3_label = Text("R = –10 N", font_size=24).next_to(reaction_arrow3, DOWN, buff=0.25)
        self.play(GrowArrow(reaction_arrow3), FadeIn(r3_label), run_time=0.5)

        # Highlight both arrows to show equality
        self.play(
            Indicate(force_arrow3, scale_factor=1.2),
            Indicate(reaction_arrow3, scale_factor=1.2),
            run_time=1.5
        )

        # Wait to reach total 10.9 s (so far 1+1+0.5+0.5+0.5+0.5+1.5 = 5.5 s)
        self.wait(5.4)