...
class OpticsVisualizer:
    ...
    def create_concave_lens(height=4.5, focal_length=2.5, *args, **kwargs):
        ...
        # Inward arrows on ends representing concave/diverging lens
...
class AutoTeach(Scene):
    def construct(self):
        ...
        header = LayoutManager.create_header(
            "Swartz Reaction (Methane Combustion)",
            "Exothermic combustion of CH₄"
        )
        self.add(header)

        # ==================== Scene 1 ====================
        # Create methane molecule
        C = MoleculeVisualizer.make_atom("C", color=BLUE_D, radius=0.25)
        H1 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
        H2 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
        H3 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
        H4 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
        # Position hydrogen atoms around carbon
        H1.move_to(C.get_center() + np.array([0.5, 0.5, 0]))
        H2.move_to(C.get_center() + np.array([-0.5, 0.5, 0]))
        H3.move_to(C.get_center() + np.array([0.5, -0.5, 0]))
        H4.move_to(C.get_center() + np.array([-0.5, -0.5, 0]))
        # Bonds
        b1 = MoleculeVisualizer.make_bond(C, H1)
        b2 = MoleculeVisualizer.make_bond(C, H2)
        b3 = MoleculeVisualizer.make_bond(C, H3)
        b4 = MoleculeVisualizer.make_bond(C, H4)
        methane = VGroup(C, H1, H2, H3, H4, b1, b2, b3, b4).shift(np.array([-3, 0, 0]))

        # Create two O₂ molecules
        def create_o2(pos):
            Oa = MoleculeVisualizer.make_atom("O", color=RED, radius=0.25)
            Ob = MoleculeVisualizer.make_atom("O", color=RED, radius=0.25)
            Oa.move_to(pos + np.array([-0.4, 0, 0]))
            Ob.move_to(pos + np.array([0.4, 0, 0]))
            bond = MoleculeVisualizer.make_bond(Oa, Ob)
            return VGroup(Oa, Ob, bond)

        O2_1 = create_o2(np.array([0, 0, 0]))
        O2_2 = create_o2(np.array([3, 0, 0]))

        # Animate appearance
        self.play(FadeIn(methane), run_time=2.0)
        self.play(FadeIn(O2_1), run_time=1.5)
        self.play(FadeIn(O2_2), run_time=1.5)
        self.wait(5.9)  # Total 10.9 seconds

        # ==================== Scene 2 ====================
        self.clear_stage(preserve=header)

        # Re‑create reactants for animation
        methane2 = methane.copy()
        O2_1b = O2_1.copy()
        O2_2b = O2_2.copy()
        self.add(methane2, O2_1b, O2_2b)

        # Vibration indication
        self.play(Indicate(methane2, scale_factor=1.05), run_time=1.0)
        self.play(Indicate(O2_1b, scale_factor=1.05), Indicate(O2_2b, scale_factor=1.05), run_time=1.0)

        # Collision motion
        self.play(
            O2_1b.animate.shift(np.array([1.2, 0, 0])),
            O2_2b.animate.shift(np.array([-1.2, 0, 0])),
            run_time=2.5
        )

        # Reaction arrow and exothermic badge
        arrow = Text("→", font_size=48).next_to(methane2, RIGHT, buff=0.7)
        badge = Text("Exothermic", font_size=36, color=YELLOW).next_to(arrow, UP, buff=0.3)
        self.play(FadeIn(arrow), FadeIn(badge), run_time=1.0)

        self.wait(4.3)  # Total 11.3 seconds

        # ==================== Scene 3 ====================
        self.clear_stage(preserve=header)

        # Reactants (again for transformation)
        reactants = VGroup(methane.copy(), O2_1.copy(), O2_2.copy()).shift(np.array([-2, 0, 0]))
        self.play(FadeIn(reactants), run_time=2.0)

        # Create product molecules
        # CO₂
        C_co2 = MoleculeVisualizer.make_atom("C", color=BLUE_D, radius=0.25)
        Oa_co2 = MoleculeVisualizer.make_atom("O", color=RED, radius=0.25)
        Ob_co2 = MoleculeVisualizer.make_atom("O", color=RED, radius=0.25)
        Oa_co2.move_to(C_co2.get_center() + np.array([-0.6, 0, 0]))
        Ob_co2.move_to(C_co2.get_center() + np.array([0.6, 0, 0]))
        bond_co2_1 = MoleculeVisualizer.make_bond(C_co2, Oa_co2)
        bond_co2_2 = MoleculeVisualizer.make_bond(C_co2, Ob_co2)
        CO2 = VGroup(C_co2, Oa_co2, Ob_co2, bond_co2_1, bond_co2_2)

        # 2 × H₂O
        def create_h2o(pos):
            O = MoleculeVisualizer.make_atom("O", color=RED, radius=0.25)
            H1 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
            H2 = MoleculeVisualizer.make_atom("H", color=WHITE, radius=0.15)
            H1.move_to(O.get_center() + np.array([0.5, 0.5, 0]))
            H2.move_to(O.get_center() + np.array([-0.5, 0.5, 0]))
            b1 = MoleculeVisualizer.make_bond(O, H1)
            b2 = MoleculeVisualizer.make_bond(O, H2)
            return VGroup(O, H1, H2, b1, b2).move_to(pos)

        H2O_1 = create_h2o(np.array([2, 1, 0]))
        H2O_2 = create_h2o(np.array([2, -1, 0]))

        products = VGroup(CO2, H2O_1, H2O_2).shift(np.array([2, 0, 0]))

        # Transform reactants into products
        self.play(Transform(reactants, products), run_time=3.0)

        # Show balanced equation
        equation = Text("CH₄ + 2 O₂ → CO₂ + 2 H₂O + ΔH", font_size=36).to_edge(DOWN)
        self.play(FadeIn(equation), run_time=2.0)

        self.wait(3.9)  # Total 10.9 seconds
        ...
        # Summary text
        summary = Text(
            "Key takeaways: the Swartz reaction is exothermic, obeys stoichiometry, and illustrates bond energy transformation",
            font_size=30
        ).to_edge(UP)

        self.play(FadeIn(summary), run_time=3.0)

        # Equation with underline and persistent badge
        equation2 = Text("CH₄ + 2 O₂ → CO₂ + 2 H₂O + ΔH", font_size=36).to_edge(DOWN)
        underline = Line(start=equation2.get_left(), end=equation2.get_right()).next_to(equation2, DOWN, buff=0.1)
        badge2 = Text("Exothermic", font_size=36, color=YELLOW).next_to(equation2, UP, buff=0.3)

        self.play(FadeIn(equation2), FadeIn(underline), FadeIn(badge2), run_time=3.0)

        self.wait(3.6)  # Total 9.6 seconds