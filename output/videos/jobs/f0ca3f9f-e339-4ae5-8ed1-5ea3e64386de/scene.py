...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ---------- Header ----------
        header = Text("Convex Lens", font_size=36, color=WHITE).to_edge(UP, buff=0.3)
        subheader = Text("How it forms real and virtual images", font_size=24, color=GRAY_A).next_to(header, DOWN, buff=0.2)
        self.play(FadeIn(header), FadeIn(subheader))
        self.wait(0.5)

        # ---------- Principal Axis ----------
        axis = Line(LEFT * 6, RIGHT * 6, color=GRAY).shift(DOWN * 0.0)
        axis_label = Text("Principal Axis", font_size=20, color=GRAY).next_to(axis, DOWN, buff=0.2)
        self.play(Create(axis), FadeIn(axis_label, shift=UP * 0.2))
        self.wait(0.3)

        # ---------- Convex Lens ----------
        left_arc = Arc(radius=1.5, start_angle=PI / 2, angle=-PI, color=WHITE)
        right_arc = Arc(radius=1.5, start_angle=-PI / 2, angle=PI, color=WHITE)
        ...
        obj_pos = LEFT * 3
        object_arrow = Arrow(
            start=obj_pos + DOWN * 0.4,
            end=obj_pos + UP * 0.4,
            buff=0,
            color=YELLOW,
        )
        obj_label = Text("Object", font_size=20, color=YELLOW).next_to(object_arrow, DOWN, buff=0.2)
        self.play(FadeIn(object_arrow), FadeIn(obj_label, shift=UP * 0.2))
        self.wait(0.5)

        # ---------- Focal Point ----------
        focal_x = 2
        focal_point = Dot(point=RIGHT * focal_x, radius=0.07, color=RED)
        focal_label = Text("F", font_size=20, color=RED).next_to(focal_point, UP, buff=0.1)
        self.play(FadeIn(focal_point), FadeIn(focal_label, shift=DOWN * 0.2))
        self.wait(0.5)

        # ---------- Light Rays ----------
        ray_ys = [0.6, 0.0, -0.6]  # top, centre, bottom
        ray_animations = []
        for y in ray_ys:
            start = np.array([obj_pos[0], y, 0])
            # point where ray meets lens surface (approximate left side of lens)
            lens_entry = np.array([-0.5, y, 0])
            # segment before lens
            pre_lens = Line(start, lens_entry, color=LIGHT_GREY)
            # segment after lens goes through focal point
            post_lens = Line(lens_entry, np.array([focal_x, 0, 0]), color=LIGHT_GREY)
            # animate creation of both segments
            ray_animations.append(Create(pre_lens))
            ray_animations.append(Create(post_lens))
        self.play(*ray_animations, run_time=2.5)
        self.wait(0.5)

        # ---------- Image Formation ----------
        img_x = 4
        image_arrow = Arrow(
            start=RIGHT * img_x + UP * 0.4,
            end=RIGHT * img_x + DOWN * 0.4,
            buff=0,
            color=GREEN,
        )
        img_label = Text("Image", font_size=20, color=GREEN).next_to(image_arrow, DOWN, buff=0.2)
        # Transform object arrow into inverted image arrow (visual cue)
        self.play(ReplacementTransform(object_arrow.copy(), image_arrow))
        self.play(FadeIn(img_label, shift=UP * 0.2))
        self.wait(0.5)

        # ---------- Magnification ----------
        mag_label = Text("m = -0.5", font_size=24, color=GREEN).next_to(image_arrow, RIGHT, buff=0.5)
        self.play(Write(mag_label))
        self.wait(0.5)

        # ---------- Formula Badge ----------
        formula_bg = RoundedRectangle(width=5, height=1, corner_radius=0.2, fill_color=BLUE_D, fill_opacity=0.8, stroke_width=0)
        formula_text = Text("1/f = 1/do + 1/di", font_size=28, color=WHITE)
        formula_badge = VGroup(formula_bg, formula_text).move_to(DOWN * 3.2)
        self.play(FadeIn(formula_badge))
        # Pulse effect
        self.play(Indicate(formula_badge, scale_factor=1.1, color=YELLOW), run_time=1.2)
        self.wait(1)

        # ---------- Status Bar ----------
        status = Text("Real image formed; rays converge at focal point", font_size=20, color=GRAY).to_edge(DOWN, buff=0.2)
        self.play(FadeIn(status, shift=UP * 0.2))
        self.wait(2)