...
class AutoTeach(Scene):
    def construct(self):
        ...
        header = LayoutManager.create_header("Newton's Third Law", "Dynamics")
        self.add(header)

        # ---------- Scene 1 ----------
        # Setup & Frame of Reference
        axes, ground = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        block = Rectangle(width=1, height=1, color=BLUE).move_to(np.array([0, 1, 0]))
        mass_label = Text("m = 2 kg", font_size=30).next_to(block, UP)
        self.play(FadeIn(axes), run_time=1.5)
        self.play(FadeIn(ground), run_time=1.0)
        self.play(FadeIn(block), run_time=1.5)
        self.play(FadeIn(mass_label), run_time=1.0)
        self.wait(5.2)  # total 10.2s
        ...
        axes, ground = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        block = Rectangle(width=1, height=1, color=BLUE).move_to(np.array([0, 1, 0]))
        force_arrow = Arrow(start=np.array([0.5, 1, 0]), end=np.array([3, 1, 0]), color=YELLOW)
        force_label = Text("F = 10 N", font_size=30, color=YELLOW).next_to(force_arrow, UP)
        self.play(FadeIn(axes), run_time=1.0)
        self.play(FadeIn(ground), run_time=0.8)
        self.play(FadeIn(block), run_time=1.0)
        self.play(FadeIn(force_arrow), FadeIn(force_label), run_time=1.5)
        self.wait(4.2)  # total 8.5s

        # ---------- Scene 3 ----------
        # Show Resulting Acceleration & Equation
        self.clear_stage(preserve=header)
        axes, ground = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        block = Rectangle(width=1, height=1, color=BLUE).move_to(np.array([0, 1, 0]))
        accel_arrow = Arrow(start=np.array([0.5, 1, 0]), end=np.array([3, 1, 0]), color=GREEN)
        accel_label = Text("a = 5 m/s²", font_size=30, color=GREEN).next_to(accel_arrow, UP)
        eq_box = Rectangle(width=4, height=1, color=WHITE).move_to(np.array([0, -2, 0]))
        eq_text = Text("F = m·a   (10 N = 2 kg·5 m/s²)", font_size=28, color=WHITE).move_to(eq_box.get_center())
        self.play(FadeIn(axes), run_time=1.0)
        self.play(FadeIn(ground), run_time=0.8)
        self.play(FadeIn(block), run_time=1.0)
        self.play(FadeIn(accel_arrow), FadeIn(accel_label), run_time=1.5)
        self.play(FadeIn(eq_box), FadeIn(eq_text), run_time=1.0)
        self.wait(6.4)  # total 11.7s

        # ---------- Scene 4 ----------
        # Illustrate Action‑Reaction Pair
        self.clear_stage(preserve=header)
        axes, ground = KinematicsVisualizer.create_canvas(x_max=10, y_max=6)
        block = Rectangle(width=1, height=1, color=BLUE).move_to(np.array([0, 1, 0]))
        force_arrow = Arrow(start=np.array([0.5, 1, 0]), end=np.array([3, 1, 0]), color=YELLOW)
        force_label = Text("F = 10 N", font_size=30, color=YELLOW).next_to(force_arrow, UP)
        reaction_arrow = Arrow(start=np.array([-0.5, 1, 0]), end=np.array([-3, 1, 0]), color=RED)
        reaction_label = Text("Reaction = -F", font_size=30, color=RED).next_to(reaction_arrow, UP)
        self.play(FadeIn(axes), run_time=1.0)
        self.play(FadeIn(ground), run_time=0.8)
        self.play(FadeIn(block), run_time=1.0)
        self.play(FadeIn(force_arrow), FadeIn(force_label), run_time=1.5)
        self.play(FadeIn(reaction_arrow), FadeIn(reaction_label), run_time=1.5)
        self.wait(5.0)  # total 10.8s