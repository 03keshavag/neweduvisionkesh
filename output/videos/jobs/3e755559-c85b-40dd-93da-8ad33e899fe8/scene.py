...
class AutoTeach(Scene):
    def construct(self):
        # Simple placeholder for DNAVisualizer
        class DNAVisualizer:
            def __init__(self, sequence):
                self.sequence = sequence

            def get_group(self):
                # Create a horizontal row of base‑pair labels as a placeholder
                groups = []
                start_x = -3.0
                for i, pair in enumerate(self.sequence):
                    txt = Text(pair, font_size=24, color=WHITE)
                    txt.move_to(np.array([start_x + i * 2.0, 0.0, 0.0]))
                    groups.append(txt)
        ...
        # ---------- Scene 1 ----------
        # Original DNA double helix ready for replication
        dna_vis = DNAVisualizer(sequence=["A‑T", "C‑G", "T‑A", "G‑C"])
        dna_group = dna_vis.get_group()
        status1 = LayoutManager.create_status_bar(
            "Original DNA double helix ready for replication."
        )
        self.play(FadeIn(dna_group), run_time=3.0)
        self.wait(2.0)
        self.play(FadeIn(status1), run_time=1.5)
        self.wait(5.0)
        ...
        # Helicase indicator
        helicase = Text("Helicase", font_size=36, color=YELLOW).move_to(
            np.array([-4.0, 0.0, 0.0])
        )
        # Replication fork marker
        fork = Text("Fork", font_size=30, color=RED).move_to(
            np.array([0.0, 0.0, 0.0])
        )
        # Simplified hydrogen bonds (as a VGroup of lines)
        bonds = VGroup(*[
            Line(
        ...
        status2 = LayoutManager.create_status_bar(
            "Helicase arrives, unzipping the strands at the replication fork."
        )

        self.play(FadeIn(helicase), run_time=1.0)
        self.play(helicase.animate.shift(np.array([2.0, 0.0, 0.0])), run_time=2.0)
        self.play(FadeOut(bonds), run_time=2.5)
        ...
        # Free complementary nucleotides (simple circles with letters)
        nuc_A = Circle(radius=0.3, color=BLUE).move_to(np.array([-3.0, -1.0, 0.0]))
        label_A = Text("A", font_size=24, color=WHITE).move_to(nuc_A.get_center())
        nuc_T = Circle(radius=0.3, color=GREEN).move_to(np.array([-2.0, -1.0, 0.0]))
        label_T = Text("T", font_size=24, color=WHITE).move_to(nuc_T.get_center())
        nuc_G = Circle(radius=0.3, color=ORANGE).move_to(np.array([-1.0, -1.0, 0.0]))
        label_G = Text("G", font_size=24, color=WHITE).move_to(nuc_G.get_center())
        nuc_C = Circle(radius=0.3, color=PURPLE).move_to(np.array([0.0, -1.0, 0.0]))
        label_C = Text("C", font_size=24, color=WHITE).move_to(nuc_C.get_center())
        free_nucs = VGroup(nuc_A, label_A, nuc_T, label_T, nuc_G, label_G, nuc_C, label_C)

        # New base pairs forming (as simple lines)
        new_pair1 = Line(
            start=np.array([-3.0, 0.5, 0.0]),
            end=np.array([-3.0, -0.5, 0.0]),
            color=YELLOW
        )
        new_pair2 = Line(
        ...
        new_pairs = VGroup(new_pair1, new_pair2, new_pair3, new_pair4)

        status3 = LayoutManager.create_status_bar(
            "Free nucleotides pair with the template strands."
        )

        self.play(FadeIn(free_nucs), run_time=1.5)
        self.play(free_nucs.animate.shift(np.array([0.0, 1.5, 0.0])), run_time=2.0)
        self.play(FadeIn(new_pairs), run_time=2.0)
        ...
        # Duplicate original helix to create two daughter helices
        daughter1 = dna_group.copy().move_to(np.array([-2.5, 0.0, 0.0]))
        daughter2 = dna_group.copy().move_to(np.array([2.5, 0.0, 0.0]))

        status4 = LayoutManager.create_status_bar(
            "Two identical daughter helices are formed."
        )

        self.play(FadeIn(daughter1), FadeIn(daughter2), run_time=2.5)
        self.play(
            Rotate(daughter1, angle=TAU/2, about_point=daughter1.get_center()),
            Rotate(daughter2, angle=-TAU/2, about_point=daughter2.get_center()),
            run_time=3.0
        )
