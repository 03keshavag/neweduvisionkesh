...
class AutoTeach(Scene):
    def construct(self):
        ...
        # --------------------------------------------------------------------
        # Header (common for all scenes)
        # --------------------------------------------------------------------
        header = LayoutManager.create_header(
            "Dijkstra's Algorithm",
            "Shortest Paths in a Weighted Graph"
        )
        self.play(FadeIn(header, shift=UP), run_time=0.6)

        # --------------------------------------------------------------------
        # SCENE 1 – Setup & Frame of Reference (9.9 seconds)
        # --------------------------------------------------------------------
        # Positions for the five cities
        positions = {
            "A": np.array([-3.0, 0.0, 0]),
            "B": np.array([-1.0, 1.0, 0]),
            "C": np.array([-1.0, -1.0, 0]),
            "D": np.array([ 2.0, 0.0, 0]),
            "E": np.array([ 4.0, 0.0, 0]),
        }
        edges = [
            ("A", "B"), ("A", "C"),
            ("B", "C"), ("B", "D"),
        ...
        graph_vis = GraphVisualizer(positions, edges, radius=0.35)
        graph = graph_vis.get_group()
        graph = LayoutManager.safe_scale(graph, max_w=11.2, max_h=5.2)

        # Highlight source node A (yellow outline)
        node_A = graph_vis.get_node("A")
        highlight_A = SurroundingRectangle(node_A, color=YELLOW, buff=0.15)

        # Show graph
        self.play(FadeIn(graph, shift=DOWN), run_time=1.5)
        # Highlight source
        self.play(Create(highlight_A), run_time=1.0)

        # Wait for the rest of the audio
        self.wait(7.4)   # 1.5 + 1.0 + 7.4 = 9.9 seconds total

        # --------------------------------------------------------------------
        # SCENE 2 – Initialize Distances & Queue (10.7 seconds)
        # --------------------------------------------------------------------
        LayoutManager.clear_stage(self, preserve=header, run_time=0.6)

        # Re‑create graph (kept for visual continuity)
        self.play(FadeIn(graph, shift=DOWN), run_time=1.0)

        # Distance labels under each node
        dist_labels = {}
        for name, pos in positions.items():
            txt = "0" if name == "A" else "∞"
            label = Text(txt, font_size=24, color=WHITE)
            label.move_to(pos + np.array([0, -0.7, 0]))
            dist_labels[name] = label
        labels_group = VGroup(*dist_labels.values())
        self.play(FadeIn(labels_group, shift=UP), run_time=1.5)

        # Priority queue box on the right side
        queue_box = RoundedRectangle(
            width=2.5, height=3.0, corner_radius=0.2,
            stroke_color=YELLOW, fill_opacity=0.1
        ).move_to(np.array([5.5, 0.0, 0]))
        queue_title = Text("Queue", font_size=20).next_to(queue_box, UP, buff=0.2)
        queue_group = VGroup(queue_box, queue_title)

        # Queue item for source A
        item_A = Text("A : 0", font_size=20, color=WHITE)
        item_A.move_to(queue_box.get_top() + np.array([0, -0.4, 0]))
        self.play(FadeIn(queue_group, shift=RIGHT), run_time=0.8)
        self.play(FadeIn(item_A, shift=RIGHT), run_time=0.8)

        # Wait remaining time
        self.wait(8.2)   # 1.0+1.5+0.8+0.8+8.2 = 10.7 seconds total

        # --------------------------------------------------------------------
        # SCENE 3 – Core Transformation & Inflection Point (11.3 seconds)
        # --------------------------------------------------------------------
        LayoutManager.clear_stage(self, preserve=header, run_time=0.6)

        # Re‑show graph and distance labels
        self.play(FadeIn(graph, shift=DOWN), run_time=0.8)
        self.play(FadeIn(labels_group, shift=UP), run_time=0.8)

        # Re‑show queue with only A inside
        self.play(FadeIn(queue_group, shift=RIGHT), run_time=0.6)
        item_A = Text("A : 0", font_size=20, color=WHITE).move_to(
            queue_box.get_top() + np.array([0, -0.4, 0])
        )
        self.play(FadeIn(item_A, shift=RIGHT), run_time=0.6)

        # ---- Extraction of A ----
        self.play(FadeOut(item_A, shift=LEFT), run_time=0.8)

        # ---- Relax edges from A ----
        edge_AB = Line(start=positions["A"], end=positions["B"], color=WHITE, stroke_width=2)
        edge_AC = Line(start=positions["A"], end=positions["C"], color=WHITE, stroke_width=2)
        self.add(edge_AB, edge_AC)
        self.play(edge_AB.animate.set_color(YELLOW), run_time=0.6)
        self.play(edge_AC.animate.set_color(YELLOW), run_time=0.6)

        # ---- Update distances for B and C ----
        old_B = dist_labels["B"]
        new_B = Text("4", font_size=24, color=WHITE).move_to(old_B.get_center())
        old_C = dist_labels["C"]
        new_C = Text("2", font_size=24, color=WHITE).move_to(old_C.get_center())

        self.play(ReplacementTransform(old_B, new_B), run_time=0.8)
        self.play(ReplacementTransform(old_C, new_C), run_time=0.8)

        # ---- Insert B and C into queue (C first) ----
        item_C = Text("C : 2", font_size=20, color=WHITE).move_to(
            queue_box.get_top() + np.array([0, -0.4, 0])
        )
        item_B = Text("B : 4", font_size=20, color=WHITE).move_to(
            queue_box.get_top() + np.array([0, -0.9, 0])
        )
        self.play(FadeIn(item_C, shift=RIGHT), run_time=0.6)
        self.play(FadeIn(item_B, shift=RIGHT), run_time=0.6)

        # Wait for the rest of the narration
        self.wait(6.5)   # total = 11.3 seconds

        # --------------------------------------------------------------------
        # SCENE 4 – Summary & Badge (15.4 seconds)
        # --------------------------------------------------------------------
        LayoutManager.clear_stage(self, preserve=header, run_time=0.6)

        # Re‑show graph, final distance labels, and queue (now with all items)
        self.play(FadeIn(graph, shift=DOWN), run_time=0.8)

        # Final distance labels (update D and E)
        old_D = dist_labels["D"]
        old_E = dist_labels["E"]
        new_D = Text("9", font_size=24, color=WHITE).move_to(old_D.get_center())
        new_E = Text("11", font_size=24, color=WHITE).move_to(old_E.get_center())
        self.play(ReplacementTransform(old_D, new_D), run_time=0.8)
        self.play(ReplacementTransform(old_E, new_E), run_time=0.8)

        # Queue now contains all nodes with final distances
        queue_items = [
            ("C", "2"), ("B", "4"), ("D", "9"), ("E", "11")
        ]
        for i, (node, dist) in enumerate(queue_items):
            txt = f"{node} : {dist}"
            itm = Text(txt, font_size=20, color=WHITE)
            itm.move_to(queue_box.get_top() + np.array([0, -0.4 - 0.5 * i, 0]))
            self.play(FadeIn(itm, shift=RIGHT), run_time=0.5)
        ...
        badge_box = RoundedRectangle(
            ...
            stroke_color=BLUE, fill_opacity=0.2
        ).move_to(np.array([0, -2.8, 0]))
        badge_text = Text(
            "Dijkstra's Algorithm – O((V+E) log V)",
            font_size=24,
            color=WHITE
        ).move_to(badge_box.get_center())
        badge = VGroup(badge_box, badge_text)
        self.play(FadeIn(badge, shift=UP), run_time=1.0)

        # Wait for the rest of the narration
        self.wait(10.8)   # total = 15.4 seconds
