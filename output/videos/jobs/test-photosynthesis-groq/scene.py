...
class AutoTeach(Scene):
    def construct(self):
        ...
        header.move_to(np.array([0, 3.2, 0]))
        self.add(header)
        ...
        status1 = LayoutManager.create_status_bar(
            "Methane and oxygen molecules ready."
        )
        status1.move_to(np.array([0, -3.0, 0]))
        self.play(FadeIn(status1), run_time=1.0)

        # Create CH4 molecule
        C = MoleculeVisualizer.make_atom("C", BLUE, 0.3).move_to(np.array([-3, 0, 0]))
        H1 = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(np.array([-3.5, 0.5, 0]))
        H2 = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(np.array([-3.5, -0.5, 0]))
        H3 = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(np.array([-2.5, 0.5, 0]))
        H4 = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(np.array([-2.5, -0.5, 0]))
        ch4_atoms = VGroup(C, H1, H2, H3, H4)
        ch4_bonds = VGroup(
            MoleculeVisualizer.make_bond(C, H1),
            MoleculeVisualizer.make_bond(C, H2),
            MoleculeVisualizer.make_bond(C, H3),
            MoleculeVisualizer.make_bond(C, H4)
        )
        ch4 = VGroup(ch4_atoms, ch4_bonds)

        # Create two O2 molecules
        O2_1_Oa = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([2, 0.3, 0]))
        O2_1_Ob = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([2.6, 0.3, 0]))
        O2_1_bond = MoleculeVisualizer.make_bond(O2_1_Oa, O2_1_Ob, double=True)
        O2_1 = VGroup(O2_1_Oa, O2_1_Ob, O2_1_bond)

        O2_2_Oa = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([2, -0.6, 0]))
        O2_2_Ob = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([2.6, -0.6, 0]))
        O2_2_bond = MoleculeVisualizer.make_bond(O2_2_Oa, O2_2_Ob, double=True)
        O2_2 = VGroup(O2_2_Oa, O2_2_Ob, O2_2_bond)

        o2_group = VGroup(O2_1, O2_2)
        ...
        # Animate appearance
        self.play(FadeIn(ch4), run_time=1.5)
        self.wait(0.3)
        self.play(FadeIn(o2_group), run_time=1.5)
        self.wait(0.3)
        self.play(FadeIn(arrow), run_time=1.0)
        self.wait(2.4)  # fill remaining time to reach 8.5s

        # ---------- Scene 2 ----------
        self.clear_stage(preserve=header)

        status2 = LayoutManager.create_status_bar(
            "Atoms vibrate and collide."
        )
        status2.move_to(np.array([0, -3.0, 0]))
        self.play(ReplacementTransform(status1, status2), run_time=1.0)

        # Re‑create molecules for animation (same positions)
        self.add(ch4, o2_group, arrow)

        # Small sinusoidal vibration (simple up‑down shift)
        self.play(
            ch4.animate.shift(UP * 0.15),
            o2_group.animate.shift(DOWN * 0.15),
            run_time=0.8
        )
        self.play(
            ch4.animate.shift(DOWN * 0.15),
            o2_group.animate.shift(UP * 0.15),
            run_time=0.8
        )
        self.play(
            ch4.animate.shift(UP * 0.15),
        ...
        # Move molecules toward the arrow centre (curved path approximated by linear shift)
        self.play(
            ch4.animate.shift(RIGHT * 2.0),
            o2_group.animate.shift(LEFT * 2.0),
            run_time=2.5
        )
        self.wait(1.4)  # fill to reach 7.8s

        # ---------- Scene 3 ----------
        self.clear_stage(preserve=header)

        status3 = LayoutManager.create_status_bar(
            "Bonds break, forming CO₂ and H₂O."
        )
        status3.move_to(np.array([0, -3.0, 0]))
        self.play(ReplacementTransform(status2, status3), run_time=1.0)

        # Fade out original reactants
        self.play(FadeOut(ch4), FadeOut(o2_group), FadeOut(arrow), run_time=1.5)
        self.wait(0.3)

        # Create CO2 molecule
        C_co2 = MoleculeVisualizer.make_atom("C", BLUE, 0.3).move_to(np.array([-1, 0, 0]))
        O_co2_a = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([-1.6, 0.4, 0]))
        O_co2_b = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(np.array([-1.6, -0.4, 0]))
        co2_bond_a = MoleculeVisualizer.make_bond(C_co2, O_co2_a, double=True)
        co2_bond_b = MoleculeVisualizer.make_bond(C_co2, O_co2_b, double=True)
        co2 = VGroup(C_co2, O_co2_a, O_co2_b, co2_bond_a, co2_bond_b)

        # Create two H2O molecules
        def make_water(center):
            O = MoleculeVisualizer.make_atom("O", GREEN, 0.25).move_to(center)
            H_left = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(center + np.array([-0.4, 0.3, 0]))
            H_right = MoleculeVisualizer.make_atom("H", RED, 0.2).move_to(center + np.array([0.4, 0.3, 0]))
            bond1 = MoleculeVisualizer.make_bond(O, H_left)
            bond2 = MoleculeVisualizer.make_bond(O, H_right)
            return VGroup(O, H_left, H_right, bond1, bond2)

        h2o1 = make_water(np.array([1, 0.6, 0]))
        h2o2 = make_water(np.array([1, -0.6, 0]))

        products = VGroup(co2, h2o1, h2o2)

        # Fade in products
        self.play(FadeIn(co2), run_time=1.5)
        self.wait(0.2)
        self.play(FadeIn(h2o1), FadeIn(h2o2), run_time=1.5)
        self.wait(0.2)

        # Exothermic badge
        exo_badge = Text("Exothermic", font_size=36, color=RED)
        exo_badge.move_to(np.array([0, 2.2, 0]))
        self.play(FadeIn(exo_badge), run_time=1.0)
        self.wait(2.7)  # fill to reach 9.9s

        # ---------- Scene 4 ----------
        self.clear_stage(preserve=header)

        status4 = LayoutManager.create_status_bar(
            "Balanced equation appears."
        )
        status4.move_to(np.array([0, -3.0, 0]))
        self.play(ReplacementTransform(status3, status4), run_time=1.0)
        ...
        equation.move_to(np.array([0, -2.0, 0]))
        self.play(FadeIn(equation), run_time=1.5)
        self.wait(0.3)

        # Re‑use exothermic badge below equation
        exo_badge2 = Text("Exothermic", font_size=30, color=RED)
        exo_badge2.next_to(equation, DOWN, buff=0.25)
        self.play(FadeIn(exo_badge2), run_time=1.0)
        self.wait(4.4)  # fill to reach 8.2s