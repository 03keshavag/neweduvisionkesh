...
class AutoTeach(Scene):
    def construct(self):
        ...
        # ---------- Header (persistent) ----------
        header = LayoutManager.create_header("Kirchhoff's Laws", "KCL & KVL")
        ...
        def make_circuit():
            # Battery
            battery = Rectangle(width=1.2, height=0.6, color=YELLOW).shift(np.array([-2, 0, 0]))
            plus = Text("+", font_size=24).next_to(battery, LEFT, buff=0.1)
            minus = Text("-", font_size=24).next_to(battery, RIGHT, buff=0.1)
            battery_grp = VGroup(battery, plus, minus)

            # Resistors
            resistor_up = Rectangle(width=1.0, height=0.4, color=BLUE).shift(np.array([0, 1, 0]))
            resistor_down = Rectangle(width=1.0, height=0.4, color=BLUE).shift(np.array([0, -1, 0]))

            # Nodes
            node_A = Dot(np.array([-3, 0, 0]), radius=0.07, color=WHITE)
            node_B = Dot(np.array([0, 0, 0]), radius=0.07, color=WHITE)
            node_C = Dot(np.array([3, 0, 0]), radius=0.07, color=WHITE)

            label_A = Text("A", font_size=24).next_to(node_A, LEFT)
            label_B = Text("B", font_size=24).next_to(node_B, UP)
            label_C = Text("C", font_size=24).next_to(node_C, RIGHT)

            # Wires
            wire_A_to_batt = Line(node_A.get_center(), battery_grp.get_left(), color=GRAY)
            wire_batt_to_C = Line(battery_grp.get_right(), node_C.get_center(), color=GRAY)

            wire_B_to_up = Line(node_B.get_center(), resistor_up.get_bottom(), color=GRAY)
            wire_B_to_down = Line(node_B.get_center(), resistor_down.get_top(), color=GRAY)

            wire_up_to_C = Line(resistor_up.get_top(), node_C.get_center(), color=GRAY)
            wire_down_to_C = Line(resistor_down.get_bottom(), node_C.get_center(), color=GRAY)

            circuit = VGroup(
                battery_grp, resistor_up, resistor_down,
                node_A, node_B, node_C,
                label_A, label_B, label_C,
                wire_A_to_batt, wire_batt_to_C,
                wire_B_to_up, wire_B_to_down,
                wire_up_to_C, wire_down_to_C,
            )
            LayoutManager.safe_scale(circuit)
            return circuit

        # ---------- Scene 1 : Setup ----------
        # Status bar for scene 1
        status1 = LayoutManager.create_status_bar("Simple series‑parallel circuit")
        status1.to_edge(DOWN, buff=0.4)

        circuit1 = make_circuit()
        self.play(FadeIn(circuit1, run_time=1.0))
