...
class AutoTeach(Scene):
    def construct(self):
        # Common utilities
        def make_queue_item(label):
            return Text(label, font_size=24).scale(0.6)

        def make_visited_badge(label):
            badge = Circle(radius=0.3, color=GREEN, fill_opacity=0.5)
            txt = Text(label, font_size=12)
            badge.add(txt)
            return badge

        # Header (kept across scenes)
        header = LayoutManager.create_header("Breadth‑First Search", "Graph Traversal")
        self.play(FadeIn(header), run_time=1.0)

        # --------------------------------------------------------------------
        # Scene 1 – Setup graph and start node (7.8 s)
        # --------------------------------------------------------------------
        positions = {
            "A": np.array([-3, 2, 0]),
            "B": np.array([-5, 0, 0]),
        ...
        graph = GraphVisualizer(positions, edges).get_group()
        self.play(Create(graph), run_time=2.0)

        # Highlight start node A in yellow
        self.play(
            graph.animate.set_vertices_color({"A": YELLOW}),
            run_time=0.8,
        )

        # Empty queue display
        queue_label = Text("Queue:", font_size=24).to_edge(DOWN).shift(LEFT * 2)
        queue_items = VGroup()
        self.play(FadeIn(queue_label), run_time=0.8)

        # Status bar (empty)
        status1 = LayoutManager.create_status_bar("Queue: ")
        self.play(FadeIn(status1), run_time=0.8)

        self.wait(2.4)  # 1.0+2.0+0.8+0.8+0.8+2.4 = 7.8 s

        # --------------------------------------------------------------------
        # Scene 2 – Enqueue and visit start node (6.8 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)

        # Re‑create graph and header for continuity
        ...
        # Queue label again
        self.play(FadeIn(queue_label), run_time=0.5)

        # Enqueue A
        item_A = make_queue_item("A").next_to(queue_label, RIGHT, buff=0.3)
        queue_items.add(item_A)
        self.play(FadeIn(item_A), run_time=0.8)

        self.wait(0.4)

        # Dequeue A → move to graph position and mark visited (green)
        self.play(item_A.animate.move_to(positions["A"]), run_time=1.2)
        self.play(
            graph.animate.set_vertices_color({"A": GREEN}),
            run_time=0.5,
        )
        badge_A = make_visited_badge("A").move_to(positions["A"])
        self.play(FadeIn(badge_A), run_time=0.6)

        # Update status bar (empty queue)
        status2 = LayoutManager.create_status_bar("Queue: ")
        self.play(ReplacementTransform(status1, status2), run_time=1.0)

        self.wait(3.0)  # 0.5+1.5+0.5+0.5+0.8+0.4+1.2+0.5+0.6+1.0+3.0 = 6.8 s

        # --------------------------------------------------------------------
        # Scene 3 – Explore neighbors of A (7.1 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)

        # Re‑create graph, header, and visited A
        self.play(FadeIn(header), run_time=0.5)
        self.play(Create(graph), run_time=1.5)
        self.play(
            graph.animate.set_vertices_color({"A": GREEN}),
        ...
        edge_AB = Line(positions["A"], positions["B"], color=YELLOW)
        edge_AC = Line(positions["A"], positions["C"], color=YELLOW)
        self.play(Create(edge_AB), Create(edge_AC), run_time=0.8)

        # Enqueue B then C
        item_B = make_queue_item("B").next_to(queue_label, RIGHT, buff=0.3)
        self.play(FadeIn(item_B), run_time=0.6)
        self.wait(0.2)
        item_C = make_queue_item("C").next_to(item_B, RIGHT, buff=0.3)
        self.play(FadeIn(item_C), run_time=0.6)

        # Temporary “queued” highlight (a faint circle) for B and C
        highlight_B = Circle(radius=0.4, color=YELLOW).move_to(positions["B"]).set_opacity(0.3)
        highlight_C = Circle(radius=0.4, color=YELLOW).move_to(positions["C"]).set_opacity(0.3)
        self.play(FadeIn(highlight_B), FadeIn(highlight_C), run_time=0.8)

        # Update status bar
        status2 = LayoutManager.create_status_bar("Queue: B, C")
        self.play(ReplacementTransform(status1, status2), run_time=1.0)

        self.wait(3.3)  # total = 7.1 s

        # --------------------------------------------------------------------
        # Scene 4 – Process remaining nodes and finish traversal (11.1 s)
        # --------------------------------------------------------------------
        self.clear_stage(preserve=header)

        # Re‑create graph, header, visited A, and queue items B, C
        self.play(FadeIn(header), run_time=0.5)
        self.play(Create(graph), run_time=1.5)
        self.play(
            graph.animate.set_vertices_color({"A": GREEN}),
        ...
        # Re‑add B and C to queue
        item_B = make_queue_item("B").next_to(queue_label, RIGHT, buff=0.3)
        item_C = make_queue_item("C").next_to(item_B, RIGHT, buff=0.3)
        self.play(FadeIn(item_B), FadeIn(item_C), run_time=1.0)

        # Dequeue B → visit
        self.play(item_B.animate.move_to(positions["B"]), run_time=1.2)
        self.play(
            graph.animate.set_vertices_color({"B": GREEN}),
            run_time=0.5,
        )
        badge_B = make_visited_badge("B").move_to(positions["B"])
        self.play(FadeIn(badge_B), run_time=0.6)

        # Enqueue D and E after B
        self.wait(0.2)
        item_D = make_queue_item("D").next_to(item_C, RIGHT, buff=0.3)
        self.play(FadeIn(item_D), run_time=0.6)
        self.wait(0.2)
        item_E = make_queue_item("E").next_to(item_D, RIGHT, buff=0.3)
        self.play(FadeIn(item_E), run_time=0.6)

        # Dequeue C → visit
        self.wait(0.4)
        self.play(item_C.animate.move_to(positions["C"]), run_time=1.2)
        self.play(
            graph.animate.set_vertices_color({"C": GREEN}),
            run_time=0.5,
        )
        badge_C = make_visited_badge("C").move_to(positions["C"])
        self.play(FadeIn(badge_C), run_time=0.6)

        # Enqueue F after C
        self.wait(0.2)
        item_F = make_queue_item("F").next_to(item_E, RIGHT, buff=0.3)
        self.play(FadeIn(item_F), run_time=0.6)

        # Final highlight of visited order A‑B‑C‑D‑E‑F
        self.wait(0.4)
        order = ["A", "B", "C", "D", "E", "F"]
        highlight = Circle(radius=0.5, color=YELLOW).set_opacity(0.5)
        for node in order:
            self.play(highlight.animate.move_to(positions[node]), run_time=0.4)
        self.play(FadeOut(highlight), run_time=0.4)

        self.wait(3.5)  # total = 11.1 s